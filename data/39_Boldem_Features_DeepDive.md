# Boldem – hloubková analýza funkcí
...
> - **Multikanálovost** (e-mail + SMS + PUSH + In-App)
> - **Agenturní účty + white-label řešení** (UNIKÁTNÍ pro CZ trh)
> - **AI-powered features** (umělá inteligence v editoru)
> - **Atraktivní cenová politika** (vs. Ecomail / SmartEmailing)
> - **Shoptet, Shopify, Upgates, Rocketoo native integrace**
> - **Limiter** (rozdělení rozesílky pro warm-up + ochrana před přetížením)
> - **Vlastní události + tagování** (custom event tracking)
> - **Power BI + Looker Studio reporty** (advanced reporting)
...
1. [Co je Boldem a pro koho je](#1-co-je-boldem)
2. [Tarify (E-mailing + SMS + CDP)](#2-tarify)
3. [Klíčové diferenciátory v CZ kontextu](#3-diferenciatory)
4. [E-mail marketing & editor](#4-email-marketing)
5. [Recipient management + seznamy](#5-recipients)
6. [Automatizace (welcome, opuštěný košík, atd.)](#6-automation)
7. [SMS hromadné + transakční](#7-sms)
8. [PUSH oznámení + In-App zprávy](#8-push)
9. [Transakční e-maily](#9-transactional)
10. [CDP (Customer Data Platform)](#10-cdp)
11. [Vlastní události + tagování](#11-custom-events)
12. [Segmentation + personalizace](#12-segmentation)
13. [A/B testing (až 4 varianty!)](#13-ab-testing)
14. [Limiter (rozdělení rozesílky)](#14-limiter)
15. [Reports + Power BI / Looker Studio](#15-reports)
16. [E-commerce integrace (Shoptet, Shopify, Upgates)](#16-integrations)
17. [API + Webhooks](#17-api)
18. [Agenturní účty + white-label](#18-agency)
19. [AI features (umělá inteligence)](#19-ai)
20. [Deliverability (vlastní servery v ČR)](#20-deliverability)
21. [Compliance, GDPR, vlastní servery v ČR](#21-compliance)
22. [Sportovní + neziskové reference](#22-references)
...
- **Společnost:** Boldem (provozovatel)
- **Předchůdce:** **MailKomplet** (klasický český e-mail tool)
- **Servery:** **Vlastní servery v České republice** (zabezpečený cloud)
- **Pozice:** **Český e-mailing + CDP** pro malé/střední e-shopy + agentury + firmy
- **Specializace:** SMB → mid-market (s CDP rozšířením)
- **Lokalizace UI:** **Čeština primary** + případně další (potřeba ověřit)
- **Web:** boldem.cz
...
- **Attractive pricing** vs. competition
...
- **Hokejový klub Slovan Bratislava**
...
```
┌─────────────────────────────────────────────────────────────────┐
│                  BOLDEM PLATFORM (české řešení)                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌────────────────┐  ┌──────────────┐  ┌─────────────────┐      │
│  │ E-mail         │  │ Hromadné     │  │ PUSH            │      │
│  │ Marketing      │  │ SMS          │  │ Oznámení        │      │
│  │ (drag-drop     │  │ + Transakční │  │ + In-App        │      │
│  │  editor + AI)  │  │              │  │ Zprávy          │      │
│  └────────────────┘  └──────────────┘  └─────────────────┘      │
│                                                                 │
│  ┌────────────────┐  ┌──────────────┐  ┌─────────────────┐      │
│  │ Transakční     │  │ Automatizace │  │ CDP             │      │
│  │ E-maily        │  │ (welcome,    │  │ (Customer Data  │      │
│  │                │  │  opuštěný    │  │  Platform)      │      │
│  │                │  │  košík, ...) │  │                 │      │
│  └────────────────┘  └──────────────┘  └─────────────────┘      │
│                                                                 │
│  ┌────────────────┐  ┌──────────────┐  ┌─────────────────┐      │
│  │ Vlastní        │  │ Tagování     │  │ Rozšířená       │      │
│  │ Události       │  │ uživatelů    │  │ Segmentace      │      │
│  │ (custom        │  │              │  │                 │      │
│  │  event track.) │  │              │  │                 │      │
│  └────────────────┘  └──────────────┘  └─────────────────┘      │
│                                                                 │
│  ┌────────────────┐  ┌──────────────┐  ┌─────────────────┐      │
│  │ A/B Testing    │  │ Limiter      │  │ Reports         │      │
│  │ (4 varianty!)  │  │ (warm-up +   │  │ + Power BI /    │      │
│  │                │  │  ochrana)    │  │ Looker Studio   │      │
│  └────────────────┘  └──────────────┘  └─────────────────┘      │
│                                                                 │
│  ┌────────────────┐  ┌──────────────┐  ┌─────────────────┐      │
│  │ Agenturní      │  │ White-label  │  │ Tržby           │      │
│  │ Účty           │  │ Řešení       │  │ z e-mailingu    │      │
│  │ (multi-client) │  │              │  │ (v aplikaci!)   │      │
│  └────────────────┘  └──────────────┘  └─────────────────┘      │
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
│   INFRASTRUKTURA:                                               │
...
│   ├─ Český tým podpory (telefon, e-mail, chat)                  │
│   └─ AI-powered features                                        │
├─────────────────────────────────────────────────────────────────┤
│   E-COMMERCE INTEGRACE (CZ nativní!):                           │
│   ├─ Shoptet (CZ leader)                                        │
│   ├─ Shopify                                                    │
│   ├─ Upgates                                                    │
│   ├─ Rocketoo                                                   │
...
│   CENA:                                                         │
│   ├─ Atraktivní cenová politika (vs. Ecomail/SmartEmailing)     │
│   ├─ Škálovatelné plány dle velikosti databáze                  │
│   ├─ Tarif Profi pro pokročilé funkce (vlastní události)        │
│   └─ CDP řešení šité na míru (pro střední/větší e-shopy)        │
└─────────────────────────────────────────────────────────────────┘
```

### Typické use cases

- **Malé až střední české e-shopy** (Shoptet/Upgates/Rocketoo/Shopify)
- **Sportovní kluby** (newslettery, prodej lístků, komunikace s fanoušky)
- **Neziskové organizace** (direct mailing, fundraising)
- **CZ/SK agentury** (multi-client management s white-label)
- **Středně velké firmy** (CDP řešení)
- **Mediální domy** (s velkou databází)
- **B2B společnosti** (lead nurturing základní)

---

## 2. Tarify (E-mailing + SMS + CDP)
...
- **Bezpečné ukládání na jedno místo**
- **Přesnější cílení**
...
```
Pricing factors:
- Velikost databáze kontaktů
- Objem e-mailů měsíčně
- SMS volume
- PUSH/In-App volume
...
- **Demo + tour**
- **Není standardní free trial** – sales-driven
...
- **Automatizace + transakce zůstávají aktivní** (důležité)
- **Kampaně se zastropí** na aktuálním tarifu
...
## 3. Klíčové diferenciátory v CZ kontextu
...
- **Existing customer base** může migrovat
...
- **Vlastní události tracking**
- **Vs. SmartEmailing/Ecomail** – CDP je explicit focus
...
- **White-label řešení**
- **UNIKÁTNÍ pro CZ** vs. Ecomail/SmartEmailing
...
- **Rychlý editor** (per reviews)

### 4.2 Kampaňový workflow
...
- Test send
- **Automatická kontrola kampaní** (spam slova, chybějící odkazy)
   ↓
Step 5: Odeslání
- Odeslat ihned
- Naplánovat
- **Limiter** (rozdělení rozesílky, Tarif Profi)
   ↓
Potvrdit
```
...
- **SmartEmailing umí sloučení i přesun**
- **Boldem přidává distributiv funkci**

---

## 5. Recipient management + seznamy
...
- **Tagging** (custom)
- **Custom fields**
- **Tarif Profi pro pokročilé**

### 5.4 Recipient status
...
## 6. Automatizace (welcome, opuštěný košík, atd.)
...
- **Neomezené automatizované procesy**
...
Trigger: Cart abandoned 1h
   ↓
Send reminder email (s obsahem košíku)
   ↓
Wait 24h
   ↓
Podmínka: Nakoupil?
   ANO → Exit (goal)
   NE → Send discount offer
   ↓
End
```
...
Track engagement
   ↓
If still inactive: Send special offer
```
...
- **Pro pokročilé uživatele** OK

---

## 7. SMS hromadné + transakční
...
- **Samostatný ceník** pro SMS
- **Per SMS pricing**
- **Volume discounts**
- **Sales-driven detaily**

### 7.5 Use v automation

```
Workflow → Send SMS node
   ↓
Personalization
   ↓
Tracking
...
- Mobile app SDK (predpokládáme iOS + Android)
...
- **E-mailové kampaně**
- **Sociální média**
- **E-shopy** (Shoptet, Shopify, Upgates, Rocketoo)
- **Mobile apps**
...
```
Klient otevře e-mail v Boldem
   ↓
Boldem CDP zaznamená:
- Engagement event
- Profile update
- Segment re-evaluation
   ↓
Současně:
- Návštěva webu (tracking script)
- Klik na produkt
- Přidání do košíku
- Opuštění košíku
   ↓
Vše centralizováno v CDP
   ↓
360° pohled na zákazníka
   ↓
Automatizace dle všech signálů
```
...
```
E-shop:
- Custom event: "premium_member_renewal"
- Tracking renewal status
- Automation: send renewal reminders

SaaS:
- Custom event: "feature_x_used"
- Tracking feature adoption
- Automation: send tutorials if not used

Sportovní klub:
- Custom event: "ticket_purchased_match_X"
- Tracking match attendance
- Automation: invite to next match
```

---

## 12. Segmentation + personalizace
...
- **Tag-based**
- **Behavior-based**
- **Event-based** (Tarif Profi)

### 12.2 Filter criteria
...
## 14. Limiter (rozdělení rozesílky)
...
```
User creates campaign s 5000 příjemci
   ↓
Aktivuje Limiter
   ↓
Konfigurace:
...
- Per-hour limit
   ↓
Send schedule:
- Dávka 1 (1000 příjemců) → ihned
- Dávka 2 (1000 příjemců) → +2 hod
- Dávka 3 (1000 příjemců) → +4 hod
- ...
   ↓
Protection + warm-up
```
...
## 15. Reports + Power BI / Looker Studio
...
## 16. E-commerce integrace (Shoptet, Shopify, Upgates)
...
3. Ověřovací kód
4. Spárování s Boldem účtem
5. Aktivace

**Shoptet je CZ e-commerce platform leader** – Boldem má native deep integration podobně jako Ecomail/SmartEmailing.

### 16.3 Shopify integrace
...
### 16.4 Upgates integrace

- **Native CZ e-shop platform**
- **Customer sync**
- **Order data**
...
- **Agency-friendly**
- **UNIKÁTNÍ pro CZ trh** vs. Ecomail/SmartEmailing

### 18.4 Agency use cases

```
Marketing agentura s 50+ klienty:
- Single Boldem agency account
- Per-client tenant / přístup
- Per-client billing (optional)
- Per-client branding (white-label)
- Per-client reports
- Agency staff:
  - Multiple uživatelé s různými právy
  - Switch mezi klienty
...
- Prodej lístků systém
- Marketing aktivity

---

## 19. AI features (umělá inteligence)
...
```
Email creation:
1. Marketer uploads logo + brand colors
2. AI detects brand identity
3. AI suggests template colors
4. AI proposes content layout
5. Marketer customizes
6. Quick deployment
```

### 19.3 AI vs. konkurence

- **Boldem AI:** focus na editor + přizpůsobení
- **Vs. Klaviyo:** méně sofistikovaná autonomy
- **Vs. HubSpot Breeze:** méně comprehensive
- **Vs. SALESmanago AI:** méně CDXP-focused
- **Pro CZ kontext:** moderní AI features

### 19.4 AI roadmap

Boldem AI je rapidly evolving - per Freshstart.cz "vyniká hlavně v přizpůsobení a využití umělé inteligence" suggests active development.

---

## 20. Deliverability (vlastní servery v ČR)
...
### 20.5 Bounce management

- **Hard bounces** auto-suppression
- **Soft bounces** retry + monitoring
- **Reputation protection**
- **Vlastní servery monitoring**

### 20.6 CZ ISP relationships
...
(Boldem CZ servery = lepší vztahy s CZ ISPs vs. zahraniční platformy)

---

## 21. Compliance, GDPR, vlastní servery v ČR
...
## 22. Sportovní + neziskové reference
...
- **Systémy na prodej lístků**
- **Marketingové aktivity**

⚠️ **UNIKÁTNÍ niche** v CZ e-mail marketing prostoru.
...
- **Q&A + networking**
- **Boldem builds community + thought leadership**
...
```
Boldem se uplatňuje v:
- E-commerce (malé a střední e-shopy)
- Sportovní kluby
- Neziskové organizace
- Direct mailing
- Agentury (white-label)
- Středně velké firmy s CDP
- Sportovní marketing
- Fundraising
```
...
### 23.3 Méně brand recognition

- **Mladší platforma** vs. Ecomail (zavedený)
- **Méně public presence** než Ecomail/SmartEmailing
- **Růstová fáze**

### 23.4 Limited public pricing transparency

- **E-mailing pricing** public
- **SMS pricing** public
- **CDP řešení** custom only
- **Méně transparentní** než MailerLite atd.

### 23.5 Sales-driven CDP

- **CDP řešení šité na míru**
- **Konzultace s obchodníkem** vyžadována
- **Méně self-serve** pro CDP

### 23.6 Méně mezinárodní

- **CZ primary**
- **Některé funkce CZ-specific**
- **Méně international** vs. Mailchimp / Brevo
- **Pro CZ/SK trh primárně**
...
### 23.8 Méně global integrations

- **Core CZ e-shop integrace** OK
- **Mezinárodní platforms** méně
- **Vs. Mailchimp / Brevo** méně integrations
- **Zapier** standardně dostupný

### 23.9 No formal CSA / ISO certification mentioned

- **Boldem nemá explicitně CSA** zmíněnou
- **Mailkit má 7 ISO certifications**
- **Boldem méně formálních certifikací**

### 23.10 Méně AI než modern competitors

- **AI editor** OK
- **Méně autonomous** než Klaviyo / HubSpot Breeze / SALESmanago AI
- **Méně predictive ML**

### 23.11 Méně landing pages native

- **Není primary landing page builder**
- **Vs. GetResponse / HubSpot / Mailchimp** komplexní landing pages
- **Workarounds** potřebné

### 23.12 Méně omnichannel hluboko

- **Email + SMS + PUSH + In-App** OK
- **Méně WhatsApp** native (Brevo má)
- **Méně Facebook Messenger** (Brevo)
- **Méně mobile push deep**

### 23.13 Méně robust webinars

- **Není webinar hosting**
- **Vs. GetResponse** komplexní webinars

### 23.14 Méně paid newsletter

- **Není podpora paid newsletter subscriptions**
- **Vs. Beehiiv / Substack / Kit**

### 23.15 Premium customer support pricing model

- **Standard podpora** OK
- **Premium / dedicated** sales-driven
...
- Provozujete **Rocketoo e-shop** (UNIKÁTNÍ integrace!)
- Cíl je **CDP** (Customer Data Platform) pro střední/větší e-shop
- Vyžadujete **multikanálovost** (email + SMS + PUSH + In-App)
- Provozujete **agenturu** s multi-client portfolio + **white-label**
- Provozujete **sportovní klub** (Boldem má strong reference!)
- Provozujete **neziskovou organizaci** s fundraisingem
- Hledáte **atraktivní cenovou politiku** vs. Ecomail/SmartEmailing
- Vyžadujete **český tým podpory** (telefon, e-mail, chat)
- Hodnotíte **4 A/B varianty** (více než Ecomail/SmartEmailing základní)
- Cíl je **Power BI / Looker Studio** advanced reporting
- Provozujete **welcome / opuštěný košík / reaktivace** automation
- Cíl je **vlastní události tracking** (Tarif Profi)
...
- Hledáte **autonomous AI agents** – Klaviyo / HubSpot
- Hledáte **deep DTC Shopify ML** – Klaviyo
- Provozujete **DACH B2B** – Inxmail / Evalanche
- Hlavní jazyk je **němčina / polština** – CleverReach / SARE
- Cíl je **deep WhatsApp + multi-channel SMS** – Brevo / SALESmanago
- Hledáte **standardní free tier** (Boldem demo only)
- Provozujete **B2B lead nurturing** primárně – ActiveCampaign / HubSpot / Evalanche
- Cíl je **omnichannel ads + on-site personalization** – SALESmanago / Bloomreach
