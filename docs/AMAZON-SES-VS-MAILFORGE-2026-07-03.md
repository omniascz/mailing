...
> **Aktualizace 2026-07-03 (fáze 1+2 „dopojení drátů") — 6 commitů, api 2068 testů zelených, `go build` ✓:**
> ✅ **E-mailové webhooky nyní firují** (`dispatchEvent` na sent/delivered/opened/clicked/bounced/complained/unsubscribed ze všech write-pointů) — největší temná mezera zavřená · ✅ **Real-time auto-pause** (bounce/complaint rate → evaluateSignal → sanctions/pause) · ✅ **Templated send** (`templateId` se renderuje) · ✅ **Per-email metering** (`recordUsage` zapojeno) · ✅ **messaging/send email** reálně odesílá · ✅ **Per-org dedicated IP** (batch-sender → sendingIp → engine) · ✅ **VERP/Return-Path** (proto regen + engine MAIL FROM + worker generuje `bounce+<msgid>@`) · ✅ **reject/rendering_failed** eventy · ✅ **account-level GetSendStatistics** · ✅ **config-set IP-pool** pro transactional · ✅ **config-set TLS-require enforcement v enginu** (proto tls_policy + pool requireTLS: reuse/dial jen TLS spojení, abort bez plaintext fallbacku; `go build`+`go vet`+smtp testy ✓) · ✅ **API-key scopes vynucené napříč** (globální guard z method+path, nerozbíjející: legacy/JWT/`*` klíče projdou, jen explicitně-scoped klíče se omezí; +katalog scopes; 2075 testů zelených). **Všechny opravitelné/infra SES mezery hotové — zbývá jen AWS moat (SNS/SQS/IAM/škála/cena), který nemá smysl honit.**
