...
| 57  | **Per-link redirect customization** (vanity short URL)                                                        | Mailchimp, Mailerlite                                                        | Místo `track.example.invalid/c/xyz` použít customer's `links.eshop.cz/c/xyz` (CNAME + SSL)                                                                                                  | M            | Vedle branded tracking domain (P0 #11); 2 dny                                                                                             |
| 58  | **Per-link clickmap UTM passthrough** (preserve customer's existing UTM on linkurl)                           | Klaviyo                                                                      | Pokud customer dá link s existujícím utm\_\*, tracking nepoškodí — append vs replace                                                                                                        | M            | UX critical pro existing analytics; 1 den                                                                                                 |
...
| 61  | **Inbound email parsing → IMAP / API webhook**                                                                | Mailgun, Postmark, Mailchimp Inbound                                         | Customer pošle email na `customer.id@inbox.example.invalid` → parse → API webhook z customer                                                                                                | M            | Mailforge má inbound-email schema + Go MX receiver; missing IMAP routing logic; 3-4 dny                                                   |
...
| 83  | **Public status page** (transparent uptime + incident log)                                                    | Instatus, Statuspage                                                         | `status.example.invalid` veřejná stránka: per-component uptime, planned maintenance, incident timeline                                                                                      | M            | Brand trust feature; Instatus $20/mo + 1 den setup; can be ready den 1                                                                    |
...
11. **Branded tracking domain** signing v URL generation chybí (hardcoded `track.example.invalid`).
...
_Konsolidováno z FORGEMSG_ROADMAP.md (901 řádků) + todonow.md (316 úkolů) + EMAIL_DEEP_ANALYSIS.md + MAILFORGE_FINDING_REPORT.md + POZICOVANI.md + TECH_STACK.md + `data/01-60` (60 souborů, 27 platforem)._
_Status: pracovní podklad pro Mailforge email layer roadmap revize před Phase 9 (closed beta) sign-off._
