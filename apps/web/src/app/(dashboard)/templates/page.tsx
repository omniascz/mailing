...
export default async function TemplatesPage({
  ...
  const [saved, builtIn] = await Promise.all([
    apiFetch<SavedTemplate[]>('/api/v1/saved-templates', { fallback: [] }),
        ...
        saved.length === 0 ? (
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <p className="mt-3 text-sm font-medium text-secondary-900">No saved templates yet</p>
              <p className="mt-1 text-sm text-secondary-500">
                Use a built-in template to seed your library, or save a campaign's HTML as a
                template (coming with the next release).
              </p>
              <Link
