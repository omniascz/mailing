...
export async function queryEvents(orgId: string, input: QueryEventsInput): Promise<CdpEvent[]> {
  ...
  if (input.since) conds.push(sql`${cdpEvents.occurredAt} >= ${sql.param(input.since, cdpEvents.occurredAt)}`);
  if (input.until) conds.push(sql`${cdpEvents.occurredAt} <= ${sql.param(input.until, cdpEvents.occurredAt)}`);
  return db
    .select()
    .from(cdpEvents)
    .where(and(...conds))
