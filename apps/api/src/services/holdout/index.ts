...
export async function populateHoldout(orgId: string, groupId: string): Promise<{ added: number }> {
  const [grp] = await db
    .select()
    .from(holdoutGroups)
    .where(and(eq(holdoutGroups.id, groupId), eq(holdoutGroups.orgId, orgId)))
    .limit(1);
...
export async function holdoutMembers(
  groupId: string,
  orgId: string,
  limit = 200,
): Promise<Array<{ contactId: string; addedAt: Date }>> {
  const [grp] = await db
    .select()
    .from(holdoutGroups)
    .where(and(eq(holdoutGroups.id, groupId), eq(holdoutGroups.orgId, orgId)))
  ...
  const rows = await db
    ...
    .where(eq(holdoutGroupMembers.groupId, groupId))
    .limit(limit);
  return rows;
}
