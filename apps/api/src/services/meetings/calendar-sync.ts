...
import {
  calendarIntegrations, calendarEvents, bookings,
  type CalendarIntegration, type Booking,
...
export interface ICalendarProvider {
  ...
  listCalendars(accessToken: string): Promise<Array<{ id: string; name: string; primary?: boolean }>>;
  fetchEvents(accessToken: string, calendarId: string, opts: {
    timeMin: Date; timeMax: Date; syncToken?: string;
...
class GoogleCalendarProvider implements ICalendarProvider {
  ...
  async refreshToken(refreshToken: string): Promise<OAuthTokenSet> {
    ...
    const token = await resp.json() as { access_token: string; expires_in: number; scope?: string };
    return {
      accessToken: token.access_token,
      refreshToken,
      expiresAt: new Date(Date.now() + token.expires_in * 1000),
  ...
  async listCalendars(accessToken: string): Promise<Array<{ id: string; name: string; primary?: boolean }>> {
  ...
  async createEvent(accessToken: string, calendarId: string, input: CreateEventInput): Promise<string> {
  ...
  async deleteEvent(accessToken: string, calendarId: string, externalEventId: string): Promise<void> {
    await fetch(
  ...
  private assertCreds(): void {
    if (!this.clientId || !this.clientSecret) throw new Error('GOOGLE_CLIENT_ID/SECRET not configured');
  }
}
...
class MicrosoftCalendarProvider implements ICalendarProvider {
  ...
  async exchangeAuthCode(code: string, redirectUri: string): Promise<OAuthTokenSet> {
    ...
    if (!resp.ok) throw AppError.unauthorized(`Microsoft token exchange failed: ${await resp.text()}`);
    const token = await resp.json() as {
      access_token: string; refresh_token?: string; expires_in: number; scope: string;
    };
  ...
  private async fetchMe(accessToken: string): Promise<{ id: string; userPrincipalName?: string; mail?: string }> {
  ...
  async listCalendars(accessToken: string): Promise<Array<{ id: string; name: string; primary?: boolean }>> {
    ...
    const data = await resp.json() as { value: Array<{ id: string; name: string; isDefaultCalendar?: boolean }> };
  ...
  async fetchEvents(
    ...
    const data = await resp.json() as {
      value: Array<{
        ...
        showAs?: string; isAllDay?: boolean;
...
export async function connectCalendar(input: {
  ...
  if (existing[0]) {
    const [row] = await db.update(calendarIntegrations).set({
      ...
      status: 'active',
      updatedAt: new Date(),
    }).where(eq(calendarIntegrations.id, existing[0].id)).returning();
    return row!;
  }

  const [row] = await db.insert(calendarIntegrations).values({
    orgId: input.orgId,
    userId: input.userId,
...
export async function listIntegrations(orgId: string, userId?: string): Promise<CalendarIntegration[]> {
  const conds = [eq(calendarIntegrations.orgId, orgId)];
  if (userId) conds.push(eq(calendarIntegrations.userId, userId));
  return db.select().from(calendarIntegrations).where(and(...conds));
}
...
export async function syncIntegration(
  ...
  const [integration] = await db.select().from(calendarIntegrations)
    .where(eq(calendarIntegrations.id, integrationId)).limit(1);
  ...
  const { events, nextSyncToken } = await p.fetchEvents(accessToken, integration.primaryCalendarId, {
    timeMin, timeMax,
  ...
  for (const e of events) {
    await db.insert(calendarEvents).values({
      ...
      set: {
        ...
        allDay: e.allDay, status: e.status, busy: e.busy,
        attendees: e.attendees, updatedAt: new Date(),
      },
    });
    imported++;
  }
...
export async function getBusyIntervals(
  ...
  const integrations = await db.select({ id: calendarIntegrations.id })
...
export async function createBooking(input: {
  ...
  if (integration && integration.primaryCalendarId && (integration.provider === 'google' || integration.provider === 'microsoft')) {
    const accessToken = await getFreshAccessToken(integration);
    externalEventId = await getCalendarProvider(integration.provider).createEvent(
      accessToken, integration.primaryCalendarId,
      {
        title: input.title,
        description: input.description,
...
export async function cancelBooking(orgId: string, bookingId: string): Promise<void> {
  ...
  if (booking.integrationId && booking.externalEventId) {
    const [integration] = await db.select().from(calendarIntegrations)
      .where(eq(calendarIntegrations.id, booking.integrationId)).limit(1);
    if (integration && integration.primaryCalendarId
