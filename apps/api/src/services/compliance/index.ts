...
const SOC2_CONTROLS: SeedControl[] = [
  // Common Criteria (CC) — required for all SOC 2 reports
  {
    ...
    description: 'Board and management set the tone for integrity.',
  },
  {
    controlId: 'CC2.1',
    category: 'CC',
    title: 'Internal communication of controls',
  ...
  },
  {
    ...
    description: 'Annual risk assessment identifies threats to the service.',
  },
  {
    controlId: 'CC4.1',
    category: 'CC',
    title: 'Monitoring of controls',
    description: 'Ongoing evaluation of control effectiveness.',
  },
  {
    controlId: 'CC5.1',
    category: 'CC',
    title: 'Control activities',
    description: 'Policies and procedures that mitigate identified risks.',
  },
  {
    controlId: 'CC6.1',
    category: 'CC',
  ...
  },
  {
    controlId: 'CC6.2',
    category: 'CC',
    title: 'User provisioning and de-provisioning',
    description: 'User access is granted based on approval; removed on termination.',
  },
  {
    controlId: 'CC6.3',
    category: 'CC',
    title: 'Role-based access',
    description: 'Privileged access (admin, owner) limited to approved users.',
  },
  {
    controlId: 'CC6.6',
    category: 'CC',
    title: 'Encryption of data in transit',
    description: 'All external data transmission uses TLS 1.2+.',
  },
  {
    controlId: 'CC6.7',
    category: 'CC',
    title: 'Encryption of data at rest',
    description: 'Database, backups, object storage encrypted at rest.',
  },
  {
    controlId: 'CC6.8',
    category: 'CC',
  ...
  },
  {
    ...
    description: 'Detection of security events via logs and alerting.',
  },
  {
    controlId: 'CC7.2',
    category: 'CC',
    title: 'Incident response',
    description: 'Documented process for responding to security incidents.',
  },
  {
    controlId: 'CC7.3',
    category: 'CC',
    title: 'Change management',
    description: 'Code changes peer-reviewed, tested, deployed via CI/CD.',
  },
  {
    controlId: 'CC8.1',
    category: 'CC',
  ...
  },
  {
    ...
    description: 'Third-party vendors assessed for security posture.',
  },
  // Availability
  {
    controlId: 'A1.1',
    ...
    description: 'System capacity monitored and forecasted.',
  },
  {
    controlId: 'A1.2',
    category: 'A',
  ...
  // Confidentiality
  {
    controlId: 'C1.1',
    category: 'C',
    title: 'Confidential data identification',
    description: 'Customer data classified and protected per classification.',
  },
  // Processing Integrity
  {
    ...
    description: 'Inputs validated before processing; errors logged and handled.',
  },
  // Privacy
  {
    controlId: 'P1.1',
    ...
    description: 'Privacy policy published and kept current (GDPR, CCPA).',
  },
  {
    controlId: 'P4.1',
    category: 'P',
    title: 'Data subject rights',
    description: 'Mechanisms for access, export, deletion of personal data.',
  },
];

export async function seedDefaultControls(orgId: string): Promise<{ inserted: number }> {
  const existing = await db
    .select({ controlId: complianceControls.controlId })
...
export async function updateControl(
  ...
  patch: Partial<
    Pick<
      ComplianceControl,
      'implemented' | 'evidenceUrl' | 'evidenceNotes' | 'ownerUserId' | 'nextReviewAt'
    >
  >,
...
export async function runAccessReview(
  ...
  const report = {
    ...
    staleNoLogin90d: active.filter(
      (u) => !u.lastLoginAt || u.lastLoginAt < new Date(Date.now() - 90 * 86_400_000),
    ).length,
    users: active.map((u) => ({
      id: u.id,
      email: u.email,
...
export async function listAccessReviews(orgId: string): Promise<AccessReview[]> {
  return db
    ...
    .orderBy(desc(accessReviews.reviewedAt))
    .limit(100);
}
...
export async function setRetentionPolicy(
  ...
  const [existing] = await db
    ...
    .where(
      and(eq(dataRetentionPolicies.orgId, orgId), eq(dataRetentionPolicies.resource, resource)),
    )
    .limit(1);
  if (existing) {
    const [updated] = await db
  ...
  const [created] = await db
    ...
    .values({ orgId, resource, retentionDays })
    .returning();
  return created!;
}
...
const RETENTION_HANDLERS: Record<string, (orgId: string, cutoff: Date) => Promise<number>> = {
  ...
  async inactive_contacts(orgId, cutoff) {
    ...
    const res = await db
      ...
      .where(
        and(eq(contacts.orgId, orgId), isNull(contacts.deletedAt), lt(contacts.createdAt, cutoff)),
      );
    return (res as unknown as { rowCount?: number }).rowCount ?? 0;
  },
};
...
export async function enforceRetention(
  orgId: string,
): Promise<Array<{ resource: string; deleted: number }>> {
...
export async function complianceReport(orgId: string): Promise<{
  ...
  const [lastReview] = await db
    .select({ reviewedAt: accessReviews.reviewedAt })
    .from(accessReviews)
    .where(eq(accessReviews.orgId, orgId))
    .orderBy(desc(accessReviews.reviewedAt))
    .limit(1);
