...
describe('bindParameters', () => {
  ...
  it('reuses the same positional param when referenced twice', () => {
    const out = bindParameters(
      'SELECT :org_id, :org_id FROM t',
      specs,
      { org_id: 'abc', since: new Date() },
    );
  ...
  it('coerces numeric string to number for number-typed params', () => {
    const out = bindParameters(
      'SELECT :limit',
      specs,
      { org_id: 'x', since: new Date(), limit: '25' },
    );
    if (!out.ok) throw new Error(out.error);
  ...
  it('rejects non-numeric string for number params', () => {
    const out = bindParameters(
      'SELECT :limit',
      specs,
      { org_id: 'x', since: new Date(), limit: 'abc' },
    );
    expect(out.ok).toBe(false);
  });
  ...
  it('coerces ISO string to Date', () => {
    const out = bindParameters(
      'SELECT :since',
      specs,
      { org_id: 'x', since: '2026-04-24T10:00:00Z' },
    );
    if (!out.ok) throw new Error(out.error);
...
describe('findUndeclaredPlaceholders', () => {
  ...
  it('returns [] when all declared', () => {
    expect(
      findUndeclaredPlaceholders('SELECT :org_id FROM t', specs),
    ).toEqual([]);
  });
});
...
describe('cacheKey', () => {
  ...
  it('differs for different values', () => {
    expect(
      cacheKey('ds-1', { org_id: 'a' }),
    ).not.toBe(cacheKey('ds-1', { org_id: 'b' }));
  });
});
