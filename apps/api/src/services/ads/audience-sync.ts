...
async function uploadToFacebookAds(account: typeof adAccounts.$inferSelect, audienceName: string, emails: string[]) {
  // Create custom audience
  const createRes = await fetch(`https://graph.facebook.com/v19.0/act_${account.platformAccountId}/customaudiences`, {
    method: 'POST',
  ...
  if (!createRes.ok || !createData.id) throw new Error(createData.error?.message ?? 'Failed to create FB audience');
...
async function uploadToGoogleAds(account: typeof adAccounts.$inferSelect, audienceName: string, _emails: string[]) {
...
async function uploadToLinkedInAds(account: typeof adAccounts.$inferSelect, audienceName: string, emails: string[]) {
  ...
  await fetch(`https://api.linkedin.com/v2/dmpSegments/${segmentId}/users`, {
    ...
    headers: { 'Authorization': `Bearer ${account.accessToken}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({
      elements: emails.map(e => ({ action: 'ADD', userIds: [{ idType: 'SHA256_EMAIL', idValue: sha256(e) }] })),
    }),
  });
...
async function uploadToTikTokAds(account: typeof adAccounts.$inferSelect, audienceName: string, _emails: string[]) {
...
export async function syncAudienceToAdPlatform(orgId: string, input: {
  adAccountId: string;
  audienceName: string;
  segmentId?: string;
  listId?: string;
  ...
  let emailQuery = db.select({ email: contacts.email }).from(contacts).where(eq(contacts.orgId, orgId));
  ...
  try {
    ...
    switch (platform) {
      case 'facebook_ads': platformAudienceId = await uploadToFacebookAds(adAccount, input.audienceName, emails); break;
      case 'google_ads':   platformAudienceId = await uploadToGoogleAds(adAccount, input.audienceName, emails); break;
      case 'linkedin_ads': platformAudienceId = await uploadToLinkedInAds(adAccount, input.audienceName, emails); break;
      case 'tiktok_ads':   platformAudienceId = await uploadToTikTokAds(adAccount, input.audienceName, emails); break;
      case 'sklik': {
      ...
      default: throw new Error(`Unsupported ad platform: ${platform}`);
    }

    const [updated] = await db
      .update(adAudienceSyncs)
      .set({ status: 'completed', platformAudienceId, contactsUploaded: emails.length, lastSyncAt: new Date(), updatedAt: new Date() })
