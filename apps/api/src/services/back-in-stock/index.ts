...
import { backInStockSubscriptions, products, type BackInStockSubscription } from '../../db/schema/index.js';
...
export async function subscribe(orgId: string, input: {
  ...
  const [row] = await db.insert(backInStockSubscriptions).values({
...
export async function listPendingForSku(orgId: string, sku: string): Promise<BackInStockSubscription[]> {
  return db.select().from(backInStockSubscriptions).where(
    and(
...
export async function listForContact(contactId: string): Promise<BackInStockSubscription[]> {
  return db.select().from(backInStockSubscriptions)
...
export async function notifyRestock(orgId: string, sku: string): Promise<RestockResult> {
  const [prod] = await db.select().from(products)
    .where(and(eq(products.orgId, orgId), eq(products.sku, sku))).limit(1);
  ...
  await db.update(backInStockSubscriptions)
...
export async function unsubscribe(id: string, orgId: string): Promise<void> {
  await db.delete(backInStockSubscriptions)
...
export async function pendingBySku(orgId: string): Promise<Array<{ sku: string; count: number }>> {
  ...
  `);
  return (rs as unknown as Array<{ sku: string; count: string }>).map((r) => ({ sku: r.sku, count: Number(r.count) }));
}
