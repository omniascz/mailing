...
async function getSenderReputation(orgId: string) {
  ...
  const warmingStatus: 'warm' | 'cooling' | 'cold' =
    openRate > 0.2 ? 'warm' :
    openRate > 0.1 ? 'cooling' : 'cold';

  return { avgOpenRate30d: openRate, avgBounceRate30d: bounceRate, avgComplaintRate30d: complaintRate, warmingStatus };
}

function ruleCheck(subject: string, html: string, fromName: string, fromEmail: string): DeliverabilityIssue[] {
  ...
  // Spam words
  const spamWords = ['free!!!', 'click here', 'act now', 'limited time', 'winner', 'guaranteed', 'risk-free', 'no obligation', 'unsubscribe all', 'opt out'];
  ...
  if (imgCount > 0 && wordCount < 30) {
    issues.push({
      ...
      explanation: 'Emails that are mostly images have poor deliverability — spam filters can\'t read image content.',
  ...
  if (fromName.length < 2 || /noreply|no-reply/i.test(fromEmail)) {
    issues.push({
      category: 'reputation', severity: 'tip',
...
export async function analyzeDeliverability(
  ...
  if (repData.avgComplaintRate30d > 0.002) {
    repIssues.push({
      category: 'reputation', severity: 'critical',
      title: `High complaint rate: ${(repData.avgComplaintRate30d * 100).toFixed(2)}%`,
      explanation: 'Spam complaint rate above 0.2% triggers ISP filtering. Gmail threshold is 0.1%.',
  ...
  if (repData.avgBounceRate30d > 0.03) {
    repIssues.push({
      category: 'reputation', severity: 'warning',
  ...
  if (repData.avgOpenRate30d > 0.2) strengths.push(`Strong open rate (${(repData.avgOpenRate30d * 100).toFixed(0)}%)`);
  ...
  const aiResult = await callClaude({
    ...
    maxTokens: 256,
  }).catch(() => ({ text: `Score ${score}/100. Focus on fixing the ${criticals > 0 ? 'critical' : 'warning'} issues first.` }));
