...
export async function listPermissionSets(orgId: string): Promise<PermissionSet[]> {
  return db
    .select()
    .from(permissionSets)
    .where(and(eq(permissionSets.orgId, orgId), isNull(permissionSets.deletedAt)));
}
...
export async function updatePermissionSet(
  ...
  const [existing] = await db
    .select()
    .from(permissionSets)
    .where(and(eq(permissionSets.id, id), eq(permissionSets.orgId, orgId)))
    .limit(1);
...
export async function deletePermissionSet(orgId: string, id: string): Promise<void> {
  ...
  await db.update(permissionSets).set({ deletedAt: new Date() }).where(eq(permissionSets.id, id));
}
...
export async function assignToUser(
  ...
  const [set] = await db
    .select()
    .from(permissionSets)
    .where(and(eq(permissionSets.id, permissionSetId), eq(permissionSets.orgId, orgId)))
    .limit(1);
