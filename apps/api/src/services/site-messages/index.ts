...
export async function createMessage(orgId: string, input: {
  siteId?: string | null;
  name: string;
  ...
  const [row] = await db.insert(siteMessages).values({
    orgId,
...
export async function getMessage(orgId: string, id: string): Promise<SiteMessage> {
  const [row] = await db.select().from(siteMessages)
    .where(and(eq(siteMessages.id, id), eq(siteMessages.orgId, orgId), isNull(siteMessages.deletedAt)));
...
export async function listMessages(orgId: string, siteId?: string): Promise<SiteMessage[]> {
  ...
  return db.select().from(siteMessages)
    .where(and(...conditions))
    .orderBy(desc(siteMessages.createdAt));
}

export async function updateMessage(orgId: string, id: string, patch: Partial<{
  name: string;
  type: SiteMessage['type'];
  trigger: SiteMessage['trigger'];
  conditions: SiteMessage['conditions'];
  ...
  const [row] = await db.update(siteMessages)
...
export async function resolveMessagesForVisitor(input: {
  ...
  const messages = await db.select().from(siteMessages).where(
    and(
  ...
  const seen = await db.select({ messageId: siteMessageImpressions.messageId })
...
export async function recordImpression(input: {
  ...
  await db.insert(siteMessageImpressions).values({
    orgId: site.orgId,
