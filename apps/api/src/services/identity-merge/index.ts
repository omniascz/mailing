...
export async function trackVisitor(orgId: string, input: {
  visitorId: string; deviceId?: string; properties?: Record<string, unknown>;
}): Promise<AnonymousProfile> {
  const [existing] = await db.select().from(anonymousProfiles)
    .where(and(eq(anonymousProfiles.orgId, orgId), eq(anonymousProfiles.visitorId, input.visitorId))).limit(1);
  if (existing) {
    const [row] = await db.update(anonymousProfiles)
      .set({
        lastSeenAt: new Date(),
  ...
  const [row] = await db.insert(anonymousProfiles).values({
    orgId, visitorId: input.visitorId,
 ...
 */
export async function mergeVisitorIntoContact(orgId: string, input: {
  visitorId: string; contactId: string;
}): Promise<MergeResult> {
  await db.update(anonymousProfiles)
    .set({ mergedInto: input.contactId, mergedAt: new Date() })
    .where(and(eq(anonymousProfiles.orgId, orgId), eq(anonymousProfiles.visitorId, input.visitorId)));

  const ev = await db.execute<{ updated: string }>(sql`
    WITH upd AS (
  ...
  const rev = await db.execute<{ updated: string }>(sql`
    WITH upd AS (
  ...
  return { visitorId: input.visitorId, contactId: input.contactId, emailEventsUpdated: evN, revenueEventsUpdated: revN };
}

export async function listUnmerged(orgId: string, limit = 100): Promise<AnonymousProfile[]> {
  return db.select().from(anonymousProfiles)
