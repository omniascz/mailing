...
export type WarehouseEntity = 'contacts' | 'email_events' | 'revenue_events' | 'campaigns' | 'workflow_runs';

const ENTITY_QUERY: Record<WarehouseEntity, (orgId: string, since: Date) => ReturnType<typeof sql>> = {
  contacts: (orgId, since) => sql`SELECT * FROM contacts WHERE org_id = ${orgId}::uuid AND updated_at >= ${since}`,
  email_events: (orgId, since) => sql`SELECT * FROM email_events WHERE org_id = ${orgId} AND created_at >= ${since}`,
  revenue_events: (orgId, since) => sql`SELECT * FROM revenue_events WHERE org_id = ${orgId}::uuid AND created_at >= ${since}`,
  campaigns: (orgId, since) => sql`SELECT * FROM campaigns WHERE org_id = ${orgId}::uuid AND updated_at >= ${since}`,
  workflow_runs: (orgId, since) => sql`SELECT * FROM workflow_runs WHERE org_id = ${orgId}::uuid AND started_at >= ${since}`,
};

export async function createWarehouseSync(orgId: string, input: {
  name: string; destination: WarehouseDestination; entities: WarehouseEntity[];
  config: Record<string, unknown>; frequency?: 'hourly' | 'daily' | 'weekly';
}): Promise<WarehouseSync> {
  if (input.entities.length === 0) throw AppError.badRequest('Pick at least one entity');
  const [row] = await db.insert(warehouseSyncs).values({
    orgId, name: input.name, destination: input.destination,
    entities: input.entities, config: input.config,
...
export async function deleteWarehouseSync(orgId: string, id: string): Promise<void> {
  await db.delete(warehouseSyncs)
...
export async function runSync(syncId: string, orgId: string): Promise<SyncRunResult> {
  const [sync] = await db.select().from(warehouseSyncs)
    .where(and(eq(warehouseSyncs.id, syncId), eq(warehouseSyncs.orgId, orgId))).limit(1);
  ...
  try {
    ...
    await db.update(warehouseSyncs).set({
      lastSyncAt: new Date(), lastStatus: 'ok', lastError: null,
    ...
    await db.update(warehouseSyncs).set({
      lastSyncAt: new Date(), lastStatus: 'error', lastError: message,
    }).where(eq(warehouseSyncs.id, sync.id));
    return { syncId: sync.id, destination: sync.destination, rowsByEntity, status: 'error', error: message };
  }
}
