...
async function validateLinkedRecords(orgId: string, input: { contactId?: string | null; accountId?: string | null }) {
  if (input.contactId) {
  ...
  if (input.accountId) {
    const [a] = await db.select({ id: accounts.id }).from(accounts)
      .where(and(eq(accounts.id, input.accountId), eq(accounts.orgId, orgId), isNull(accounts.deletedAt)));
    if (!a) throw AppError.badRequest('Account not found');
  }
}

export async function createDeal(orgId: string, input: {
  pipelineId: string;
  stageId?: string;
  name: string;
  description?: string;
  ...
  await db.insert(dealStageHistory).values({
    orgId, dealId: deal.id, fromStageId: null, toStageId: stageId,
  });

  if (deal.contactId) {
    await onApiEvent(orgId, deal.contactId, 'deal_created', { dealId: deal.id, value: Number(deal.value), stageId });
  }
  return deal;
}
...
export async function listDeals(orgId: string, opts: {
  pipelineId?: string;
  stageId?: string;
...
export async function updateDeal(orgId: string, id: string, patch: {
  name?: string;
  description?: string;
  contactId?: string | null;
  accountId?: string | null;
  ...
  if (patch.value !== undefined) update.value = typeof patch.value === 'number' ? String(patch.value) : patch.value;
  ...
  void existing; void changedByUserId;
  return row!;
}

export async function moveDealToStage(orgId: string, id: string, newStageId: string, changedByUserId?: string) {
  ...
  await db.insert(dealStageHistory).values({
    orgId, dealId: id,
    fromStageId: deal.stageId, toStageId: newStageId,
    durationSeconds, changedByUserId: changedByUserId ?? null,
  });
  ...
  if (updated!.contactId) {
    await onApiEvent(orgId, updated!.contactId, 'deal_stage_changed', {
      dealId: id, fromStageId: deal.stageId, toStageId: newStageId, probability: newStage.probability,
    });
  }
...
export async function markDealWon(orgId: string, id: string, closeDate?: Date, changedByUserId?: string) {
  const deal = await getDeal(orgId, id);
  ...
  if (updated!.contactId) {
    await onApiEvent(orgId, updated!.contactId, 'deal_won', { dealId: id, value: Number(updated!.value) });
  }
  void changedByUserId;
  return updated!;
}

export async function markDealLost(orgId: string, id: string, reason: string, closeDate?: Date, changedByUserId?: string) {
  const deal = await getDeal(orgId, id);
  if (deal.status !== 'open') throw AppError.badRequest('Deal is already closed');
...
export async function dealStageHistoryForDeal(orgId: string, dealId: string) {
  ...
  return db.select().from(dealStageHistory)
