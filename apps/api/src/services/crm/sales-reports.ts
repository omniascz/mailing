...
export async function dealsByStage(orgId: string, pipelineId: string): Promise<StageDistribution[]> {
...
export async function conversionRates(orgId: string, pipelineId: string, days = 90): Promise<ConversionRate[]> {
  const pipeline = await getPipeline(orgId, pipelineId);
...
export async function salesVelocity(orgId: string, pipelineId: string, days = 180): Promise<VelocityStats> {
  const since = new Date(Date.now() - days * 86400_000);
  const rows = await db.select({
    status: deals.status,
  ...
  let totalWon = 0, countWon = 0, totalLost = 0, countLost = 0;
  for (const r of rows) {
    if (!r.actualCloseDate) continue;
    const deltaDays = (r.actualCloseDate.getTime() - r.createdAt.getTime()) / 86400_000;
    if (r.status === 'won') { totalWon += deltaDays; countWon++; }
    if (r.status === 'lost') { totalLost += deltaDays; countLost++; }
  }
...
export async function repPerformance(orgId: string, days = 180): Promise<RepPerformance[]> {
  ...
  const byOwner = new Map<string | null, { open: number; won: number; lost: number; wonValue: number }>();
  for (const r of rows) {
    const key = r.ownerUserId;
    const entry = byOwner.get(key) ?? { open: 0, won: 0, lost: 0, wonValue: 0 };
    if (r.status === 'open') entry.open++;
    if (r.status === 'won') { entry.won++; entry.wonValue += Number(r.value); }
    if (r.status === 'lost') entry.lost++;
    byOwner.set(key, entry);
  }

  return [...byOwner.entries()].map(([ownerUserId, e]) => {
...
export async function winLossAnalysis(orgId: string, days = 180): Promise<WinLossAnalysis> {
  ...
  const bySource = [...sourceMap.entries()].map(([source, e]) => ({
    source,
    won: e.won,
    lost: e.lost,
