...
export async function createAccount(orgId: string, input: Omit<AccountInsert, 'id' | 'orgId' | 'createdAt' | 'updatedAt' | 'deletedAt'>) {
  if (input.parentAccountId) {
    const [parent] = await db.select({ id: accounts.id })
      .from(accounts)
      .where(and(eq(accounts.id, input.parentAccountId), eq(accounts.orgId, orgId), isNull(accounts.deletedAt)));
    if (!parent) throw AppError.badRequest('Parent account not found');
  }

  const [row] = await db.insert(accounts).values({ ...input, orgId }).returning();
  return row!;
}
...
export async function listAccounts(orgId: string, opts: {
  search?: string;
  industry?: string;
  ownerUserId?: string;
  limit?: number;
  ...
  if (search) conditions.push(sql`(${accounts.name} ILIKE ${'%' + search + '%'} OR ${accounts.domain} ILIKE ${'%' + search + '%'})`);
...
export async function updateAccount(orgId: string, id: string, patch: Partial<Omit<AccountInsert, 'id' | 'orgId' | 'createdAt'>>) {
...
export async function listChildAccounts(orgId: string, parentId: string) {
  return db.select().from(accounts)
    .where(and(eq(accounts.orgId, orgId), eq(accounts.parentAccountId, parentId), isNull(accounts.deletedAt)))
