...
const SPAM_WORDS = ['free!!!', 'winner', 'guaranteed', 'no obligation', 'risk-free', 'click here', 'act now', 'limited time offer'];

function detectSignals(subject: string, html: string, _fromEmail: string): Partial<PlacementSignals> {
  ...
  return {
    subjectHasSpamWords: SPAM_WORDS.some((w) => subject.toLowerCase().includes(w) || text.toLowerCase().includes(w)),
...
async function getSenderStats(orgId: string) {
  ...
  }).from(emailEvents).where(and(eq(emailEvents.orgId, orgId), gte(emailEvents.createdAt, d30)));
...
function predictProvider(provider: string, signals: PlacementSignals): ProviderResult {
  ...
  if (signals.complaintRate30d > 0.002) { spamScore += 30; reasons.push(`High complaint rate (${(signals.complaintRate30d * 100).toFixed(2)}%)`); }
  if (signals.bounceRate30d > 0.05)     { spamScore += 20; reasons.push(`High bounce rate (${(signals.bounceRate30d * 100).toFixed(1)}%)`); }
  if (!signals.hasDmarc)                { spamScore += 10; reasons.push('No DMARC policy'); }
  if (!signals.hasSpfDkim)              { spamScore += 15; reasons.push('SPF/DKIM not verified'); }
  if (signals.subjectHasSpamWords)      { spamScore += 20; reasons.push('Spam trigger words in subject/body'); }
  if (signals.missingUnsubscribe)       { spamScore += 25; reasons.push('Missing unsubscribe link'); }
  if (signals.imageToTextRatio > 0.7)   { spamScore += 10; reasons.push(`Image-heavy email (${(signals.imageToTextRatio * 100).toFixed(0)}% images)`); }

  // Promotions signals (Gmail-specific)
  if (provider === 'gmail') {
    if (signals.openRate30d < 0.15)   { promotionsScore += 20; reasons.push('Low engagement signals promotions tab placement'); }
    if (signals.linkCount > 5)         { promotionsScore += 10; reasons.push('Many links suggest promotional content'); }
    if (signals.htmlSizekb > 100)      { promotionsScore += 10; reasons.push('Large HTML email'); }
  }
...
export async function simulateInboxPlacement(
  ...
  const overallScore = Math.round(100 - (spamCount / providers.length) * 60 - (senderStats.complaintRate30d * 1000));

  const issues: string[] = [];
  const recommendations: string[] = [];
  if (signals.missingUnsubscribe) { issues.push('Missing unsubscribe link'); recommendations.push('Add a clearly visible unsubscribe link'); }
  if (signals.complaintRate30d > 0.001) { issues.push('Elevated spam complaint rate'); recommendations.push('Review list hygiene and targeting'); }
  if (!signals.hasDmarc) { issues.push('DMARC not configured'); recommendations.push('Set up DMARC policy for your sending domain'); }
  if (signals.imageToTextRatio > 0.6) { issues.push('High image-to-text ratio'); recommendations.push('Add more text content to improve deliverability'); }
  if (inboxCount >= 3) recommendations.push('Good inbox placement expected — keep current practices');

  const [row] = await db.insert(inboxPlacementTests).values({
    orgId,
