...
export async function autoAssignTicket(
  ...
  const rule = rules.find((r) => r.isActive && (!r.channel || r.channel === channel));
  ...
  const candidates = available.filter((a) => {
    ...
    const needed =
      requiredSkills.length > 0 ? requiredSkills : rule?.requiredSkill ? [rule.requiredSkill] : [];
    if (needed.length > 0 && !needed.every((s) => a.skills.includes(s))) return false;
    // Agent whitelist
    if (rule?.agentIds && rule.agentIds.length > 0 && !rule.agentIds.includes(a.userId))
      return false;
    return true;
  });
...
export async function manualAssignTicket(
  ...
  const [ticket] = await db
    .select({ assignedTo: helpdeskTickets.assignedTo })
  ...
  if (ticket.assignedTo && ticket.assignedTo !== assignedTo) {
    await db
      .update(agentAvailability)
      .set({
        activeCount: sql`GREATEST(${agentAvailability.activeCount} - 1, 0)`,
        updatedAt: new Date(),
      })
      .where(
        and(eq(agentAvailability.orgId, orgId), eq(agentAvailability.userId, ticket.assignedTo)),
      );
  }
  ...
  await db
    ...
    .values({ orgId, ticketId, assignedTo, assignedBy, reason: 'manual' });
}
