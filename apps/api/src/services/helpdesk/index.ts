...
import {
  helpdeskTickets, ticketMessages,
...
export async function openTicket(orgId: string, input: {
  subject: string; contactId?: string; channel?: string;
  priority?: 'low' | 'normal' | 'high' | 'urgent';
  body: string; tags?: string[];
}): Promise<HelpdeskTicket> {
  const [ticket] = await db.insert(helpdeskTickets).values({
    orgId, subject: input.subject,
...
export async function listTickets(orgId: string, opts?: {
  status?: TicketStatus; limit?: number;
}): Promise<HelpdeskTicket[]> {
  const conds = [eq(helpdeskTickets.orgId, orgId)];
  if (opts?.status) conds.push(eq(helpdeskTickets.status, opts.status));
  return db.select().from(helpdeskTickets).where(and(...conds))
    .orderBy(desc(helpdeskTickets.updatedAt))
    .limit(opts?.limit ?? 50);
}

export async function getTicket(orgId: string, ticketId: string): Promise<{
  ticket: HelpdeskTicket; messages: TicketMessage[];
}> {
...
export async function appendMessage(orgId: string, ticketId: string, input: {
  sender: 'customer' | 'agent' | 'system'; body: string;
  ...
  const [msg] = await db.insert(ticketMessages).values({
    ticketId, sender: input.sender, body: input.body,
    attachments: input.attachments ?? [],
  }).returning();
  await db.update(helpdeskTickets).set({ updatedAt: new Date() })
    .where(eq(helpdeskTickets.id, ticketId));
  return msg!;
}

export async function updateStatus(orgId: string, ticketId: string, status: TicketStatus): Promise<HelpdeskTicket> {
  const [row] = await db.update(helpdeskTickets)
    .set({
      status,
...
export async function assignTicket(orgId: string, ticketId: string, userId: string | null): Promise<HelpdeskTicket> {
  const [row] = await db.update(helpdeskTickets)
