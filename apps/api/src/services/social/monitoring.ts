...
export async function addMonitoringKeyword(orgId: string, keyword: string, platforms: string[] = []) {
  const [row] = await db
...
export async function listMonitoringKeywords(orgId: string) {
  return db.select().from(socialMonitoringKeywords).where(eq(socialMonitoringKeywords.orgId, orgId));
}

export async function removeMonitoringKeyword(orgId: string, keywordId: string) {
  const [row] = await db
    .delete(socialMonitoringKeywords)
    .where(and(eq(socialMonitoringKeywords.orgId, orgId), eq(socialMonitoringKeywords.id, keywordId)))
...
export async function ingestMention(orgId: string, input: {
  platform: string;
...
export async function runMonitoringPoll(orgId: string) {
  ...
  for (const kw of keywords) {
    const platforms = (kw.platforms as string[]).length > 0 ? kw.platforms as string[] : ['twitter'];
...
export async function listMentions(orgId: string, opts?: { status?: string; platform?: string; limit?: number }) {
  return db
    .select()
    .from(socialMentions)
