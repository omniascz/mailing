...
async function publishToFacebook(account: typeof socialAccounts.$inferSelect, caption: string, mediaUrls: string[]): Promise<PublishResult> {
  ...
  if (!res.ok || json.error) {
    return { accountId: account.id, platformPostId: null, url: null, error: json.error?.message ?? 'Unknown error' };
  }
  return {
...
async function publishToInstagram(account: typeof socialAccounts.$inferSelect, caption: string, mediaUrls: string[]): Promise<PublishResult> {
  if (!mediaUrls.length) {
    return { accountId: account.id, platformPostId: null, url: null, error: 'Instagram requires at least one media URL' };
  }
  // Step 1: create media container
  const containerRes = await fetch(`https://graph.facebook.com/v19.0/${account.platformUserId}/media`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  if (!containerRes.ok || !container.id) {
    return { accountId: account.id, platformPostId: null, url: null, error: container.error?.message ?? 'Media container creation failed' };
  }
  // Step 2: publish
  const publishRes = await fetch(`https://graph.facebook.com/v19.0/${account.platformUserId}/media_publish`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
async function publishToLinkedIn(account: typeof socialAccounts.$inferSelect, caption: string): Promise<PublishResult> {
  const body = {
...
async function publishToTwitter(account: typeof socialAccounts.$inferSelect, caption: string): Promise<PublishResult> {
...
async function publishToTikTok(account: typeof socialAccounts.$inferSelect, caption: string, mediaUrls: string[]): Promise<PublishResult> {
  if (!mediaUrls.length) {
    return { accountId: account.id, platformPostId: null, url: null, error: 'TikTok requires video URL' };
  }
...
async function publishToAccount(account: typeof socialAccounts.$inferSelect, caption: string, mediaUrls: string[]): Promise<PublishResult> {
  const platform = account.platform as SocialPlatform;
  switch (platform) {
    case 'facebook': return publishToFacebook(account, caption, mediaUrls);
    case 'instagram': return publishToInstagram(account, caption, mediaUrls);
    case 'linkedin': return publishToLinkedIn(account, caption);
    case 'twitter': return publishToTwitter(account, caption);
    case 'tiktok': return publishToTikTok(account, caption, mediaUrls);
    default: return { accountId: account.id, platformPostId: null, url: null, error: `Unknown platform: ${platform}` };
  }
}

// ── CRUD ──────────────────────────────────────────────────────────────────────

export async function createSocialPost(orgId: string, input: {
  caption?: string;
  mediaUrls?: string[];
  linkUrl?: string;
  hashtags?: string[];
...
export async function listSocialPosts(orgId: string, opts?: { status?: string; from?: Date; to?: Date; limit?: number }) {
  ...
  if (opts?.status) q = q.where(drizzleAnd(drizzleEq(socialPosts.orgId, orgId), drizzleEq(socialPosts.status, opts.status)));
...
export async function getSocialPost(orgId: string, postId: string) {
  const [row] = await db.select().from(socialPosts).where(and(eq(socialPosts.orgId, orgId), eq(socialPosts.id, postId)));
  if (!row) throw AppError.notFound('Post not found');
  return row;
}

export async function updateSocialPost(orgId: string, postId: string, input: {
  caption?: string;
  mediaUrls?: string[];
  hashtags?: string[];
  accountIds?: string[];
...
export async function deleteSocialPost(orgId: string, postId: string) {
  const [row] = await db.delete(socialPosts).where(and(eq(socialPosts.orgId, orgId), eq(socialPosts.id, postId))).returning({ id: socialPosts.id });
  if (!row) throw AppError.notFound('Post not found');
}
...
export async function publishSocialPost(orgId: string, postId: string) {
  ...
  const accounts = await Promise.all(
    accountIdList.map(id =>
      db.select().from(socialAccounts).where(and(eq(socialAccounts.id, id), eq(socialAccounts.orgId, orgId))).then(r => r[0])
    )
  );
  const validAccounts = accounts.filter(Boolean);

  const caption = [post.caption ?? '', ...(post.hashtags as string[]).map(h => `#${h}`)].filter(Boolean).join(' ');
...
export async function dispatchDuePosts() {
  ...
  for (const post of due) {
    try {
      ...
      await db.update(socialPosts).set({ status: 'failed', updatedAt: new Date() }).where(eq(socialPosts.id, post.id));
