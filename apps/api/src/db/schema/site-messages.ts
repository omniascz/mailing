...
export const siteMessages = pgTable('site_messages', {
  ...
  index('site_messages_org_idx').on(t.orgId),
  index('site_messages_site_idx').on(t.siteId),
]);

/** Tracks which visitors have already seen a message */
export const siteMessageImpressions = pgTable('site_message_impressions', {
  id: uuid('id').primaryKey().default(sql`gen_random_uuid()`),
  orgId: uuid('org_id').notNull(),
  messageId: uuid('message_id').notNull().references(() => siteMessages.id, { onDelete: 'cascade' }),
