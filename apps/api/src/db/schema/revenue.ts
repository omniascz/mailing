...
export const revenueEvents = pgTable(
  'revenue_events',
  {
    ...
    items: jsonb('items').$type<Array<{ sku: string; name: string; qty: number; price: number }>>().notNull().default([]),
    utmSource: varchar('utm_source', { length: 100 }),
    utmMedium: varchar('utm_medium', { length: 100 }),
    utmCampaign: varchar('utm_campaign', { length: 100 }),
    attributedCampaignId: uuid('attributed_campaign_id').references(() => campaigns.id, { onDelete: 'set null' }),
