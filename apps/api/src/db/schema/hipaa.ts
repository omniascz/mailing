...
export const businessAssociateAgreements = pgTable('business_associate_agreements', {
  ...
  index('baa_status_idx').on(t.orgId, t.status),
]);

/** Marks specific custom field keys as containing PHI */
export const phiFieldFlags = pgTable('phi_field_flags', {
  id: uuid('id').primaryKey().default(sql`gen_random_uuid()`),
  orgId: uuid('org_id').notNull().references(() => organizations.id, { onDelete: 'cascade' }),
  ...
  index('phi_fields_org_idx').on(t.orgId),
]);

/** HIPAA access log for PHI access events (§164.312(b) — audit controls) */
export const hipaaAccessLog = pgTable('hipaa_access_log', {
  id: uuid('id').primaryKey().default(sql`gen_random_uuid()`),
  orgId: uuid('org_id').notNull().references(() => organizations.id, { onDelete: 'cascade' }),
