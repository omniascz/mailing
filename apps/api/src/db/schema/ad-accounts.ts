...
export const AD_PLATFORMS = ['facebook_ads', 'google_ads', 'linkedin_ads', 'tiktok_ads', 'sklik'] as const;
...
export const adAccounts = pgTable('ad_accounts', {
  ...
  orgPlatformUq: uniqueIndex('ad_accounts_org_platform_uq').on(t.orgId, t.platform, t.platformAccountId),
  orgActiveIdx:  index('ad_accounts_org_active_idx').on(t.orgId, t.active),
}));

// Records of audience syncs
export const adAudienceSyncs = pgTable('ad_audience_syncs', {
  id:              uuid('id').primaryKey().defaultRandom(),
  orgId:           uuid('org_id').notNull().references(() => organizations.id, { onDelete: 'cascade' }),
