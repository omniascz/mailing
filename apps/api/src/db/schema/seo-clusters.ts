...
export const seoTopicClusters = pgTable(
  'seo_topic_clusters',
  {
...
export const seoClusterPages = pgTable(
  'seo_cluster_pages',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    orgId: uuid('org_id')
    ...
    clusterId: uuid('cluster_id')
      .notNull()
      .references(() => seoTopicClusters.id, { onDelete: 'cascade' }),
...
export const seoKeywords = pgTable(
  ...
  (t) => ({
    orgKeywordUq: uniqueIndex('seo_keywords_org_kw_uq').on(
      t.orgId,
      t.keyword,
      t.language,
      t.country,
...
export const seoRankTracking = pgTable(
  'seo_rank_tracking',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    orgId: uuid('org_id')
...
export const seoRankSnapshots = pgTable(
  'seo_rank_snapshots',
  {
    ...
    trackingId: uuid('tracking_id')
      .notNull()
      .references(() => seoRankTracking.id, { onDelete: 'cascade' }),
