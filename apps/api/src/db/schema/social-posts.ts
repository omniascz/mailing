...
export const socialMentions = pgTable('social_mentions', {
  ...
  orgPlatformMentionUq: index('social_mentions_uq_idx').on(t.orgId, t.platform, t.platformMentionId),
...
export const socialMonitoringKeywords = pgTable('social_monitoring_keywords', {
  id:        uuid('id').primaryKey().defaultRandom(),
  orgId:     uuid('org_id').notNull().references(() => organizations.id, { onDelete: 'cascade' }),
