...
export async function exchangeCode(input: {
  ...
  const res = await fetch(TOKEN_URL(host), {
    method: 'POST',
...
async function refreshAccessToken(
  ...
  if (!conn.refreshToken)
    throw AppError.badRequest('Salesforce connection has no refresh token; reauthorize required');
  const body = new URLSearchParams({
  ...
  const res = await fetch(TOKEN_URL(DEFAULT_LOGIN_HOST), {
    method: 'POST',
    headers: { 'content-type': 'application/x-www-form-urlencoded' },
    body,
  });
...
export async function queryContacts(
  ...
  sinceIso?: string,
  limit = 200,
): Promise<SfQueryResult<SfContactRecord>> {
...
export async function queryAccounts(
  ...
  sinceIso?: string,
  limit = 200,
): Promise<SfQueryResult<SfAccountRecord>> {
  const where = sinceIso ? `WHERE LastModifiedDate > ${sinceIso}` : '';
...
export async function queryOpportunities(
  ...
  sinceIso?: string,
  limit = 200,
): Promise<SfQueryResult<SfOpportunityRecord>> {
  const where = sinceIso ? `WHERE LastModifiedDate > ${sinceIso}` : '';
...
export async function upsertContact(
  conn: SalesforceConnection,
  body: Partial<SfContactRecord>,
  externalId?: { field: string; value: string },
): Promise<{ id: string }> {
  if (externalId) {
    const path = `/sobjects/Contact/${externalId.field}/${encodeURIComponent(externalId.value)}`;
    const res = await sfFetch<{ id: string }>(conn, path, {
      method: 'PATCH',
  ...
  if (body.Id) {
    await sfFetch(conn, `/sobjects/Contact/${body.Id}`, {
      method: 'PATCH',
      body: JSON.stringify({ ...body, Id: undefined }),
    });
    return { id: body.Id };
  }
  return sfFetch<{ id: string }>(conn, '/sobjects/Contact', {
    method: 'POST',
    body: JSON.stringify(body),
  });
}
...
export async function upsertAccount(
  conn: SalesforceConnection,
  body: Partial<SfAccountRecord>,
): Promise<{ id: string }> {
  if (body.Id) {
    await sfFetch(conn, `/sobjects/Account/${body.Id}`, {
      method: 'PATCH',
      body: JSON.stringify({ ...body, Id: undefined }),
    });
    return { id: body.Id };
  }
  return sfFetch<{ id: string }>(conn, '/sobjects/Account', {
    method: 'POST',
    body: JSON.stringify(body),
  });
}
...
export async function upsertOpportunity(
  conn: SalesforceConnection,
  body: Partial<SfOpportunityRecord>,
): Promise<{ id: string }> {
  if (body.Id) {
    await sfFetch(conn, `/sobjects/Opportunity/${body.Id}`, {
      method: 'PATCH',
      body: JSON.stringify({ ...body, Id: undefined }),
    });
    return { id: body.Id };
  }
  return sfFetch<{ id: string }>(conn, '/sobjects/Opportunity', {
    method: 'POST',
    body: JSON.stringify(body),
  });
}
