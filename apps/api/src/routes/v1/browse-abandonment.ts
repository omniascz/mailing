...
const browseAbandonmentRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      if (!body.contactId && !body.visitorToken) {
        return reply
          .code(400)
          .send({ code: 'MISSING_IDENTITY', message: 'contactId or visitorToken required' });
      }
  ...
  app.get(
    '/api/v1/browse/contact/:contactId',
    {
      preHandler: [app.authenticate],
    ...
    async (req, reply) => {
      ...
      const { sinceHours } = z
        .object({ sinceHours: z.coerce.number().int().min(1).max(8760).default(72) })
        .parse(req.query);
  ...
  app.get(
    '/api/v1/browse/top-products',
    {
      preHandler: [app.authenticate],
      schema: {
        tags: ['Browse Abandonment'],
        summary: 'Top viewed products (retargeting dashboard)',
      },
    },
    async (req, reply) => {
      const { limit } = z
  ...
  app.post(
    '/api/v1/browse/check-abandonment',
    {
      preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
      schema: {
        tags: ['Browse Abandonment'],
        summary: 'Trigger browse-abandonment detection (run or test)',
      },
    },
    async (req, reply) => {
      const body = z
  ...
  app.post(
    '/api/v1/browse/converted',
    {
      preHandler: [app.authenticate],
      schema: {
        tags: ['Browse Abandonment'],
        summary: 'Mark contact as purchased (suppress retargeting)',
      },
    },
    async (req, reply) => {
  ...
  app.post(
    '/api/v1/browse/associate',
    {
      preHandler: [app.authenticate],
      schema: {
        tags: ['Browse Abandonment'],
        summary: 'Backfill anonymous page views to a contact',
      },
    },
    async (req, reply) => {
      const body = z
        ...
        .parse(req.body);
      const updated = await associateVisitorViews(
        req.user!.orgId,
        body.visitorToken,
        body.contactId,
      );
