...
import { newsletterPlans, newsletterPaidSubscriptions as newsletterSubscriptions } from '../../db/schema/newsletter-paywall.js';
...
export default async function newsletterPaywallRoutes(app: FastifyInstance) {
  ...
  app.get('/api/v1/newsletter-paywall/plans', async (req) => {
    const rows = await db.select().from(newsletterPlans)
  ...
  app.post('/api/v1/newsletter-paywall/plans', async (req, reply) => {
    ...
    const [row] = await db.insert(newsletterPlans).values({
  ...
  app.put('/api/v1/newsletter-paywall/plans/:id', async (req) => {
    ...
    const [row] = await db.update(newsletterPlans)
      .set(patch as never)
  ...
  app.delete('/api/v1/newsletter-paywall/plans/:id', async (req, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    await db.update(newsletterPlans)
      .set({ active: false })
      .where(and(eq(newsletterPlans.id, id), eq(newsletterPlans.orgId, req.user!.orgId)));
  ...
  app.get('/api/v1/newsletter-paywall/subscriptions', async (req) => {
    const q = z.object({ planId: z.string().uuid().optional(), status: z.string().optional() }).parse(req.query);
  ...
  app.post('/api/v1/newsletter-paywall/subscriptions', async (req, reply) => {
    ...
    const [row] = await db.insert(newsletterSubscriptions).values({
      ...
      target: [newsletterSubscriptions.orgId, newsletterSubscriptions.contactId, newsletterSubscriptions.planId],
      set: { status: 'active', stripeSubscriptionId: body.stripeSubscriptionId, updatedAt: new Date() },
    }).returning();
  ...
  app.get('/api/v1/newsletter-paywall/check-access', async (req) => {
    const q = z.object({ contactId: z.string().uuid(), requiredTier: z.enum(['free','basic','premium','vip']) }).parse(req.query);
    const tierOrder = ['free', 'basic', 'premium', 'vip'];
    const requiredLevel = tierOrder.indexOf(q.requiredTier);

    const subs = await db.select({ tier: newsletterSubscriptions.tier })
  ...
  app.post('/api/v1/newsletter-paywall/subscriptions/:id/cancel', async (req) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    const [row] = await db.update(newsletterSubscriptions)
      .set({ status: 'cancelled', cancelledAt: new Date(), updatedAt: new Date() })
      .where(and(eq(newsletterSubscriptions.id, id), eq(newsletterSubscriptions.orgId, req.user!.orgId)))
      .returning();
    return { data: row };
  });
}
