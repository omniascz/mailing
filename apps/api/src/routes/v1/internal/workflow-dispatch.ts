...
async function resolveOrgFrom(orgId: string): Promise<{ fromName: string; fromEmail: string } | null> {
  const [dom] = await db
...
export default async function internalWorkflowDispatchRoutes(app: FastifyInstance) {
  ...
  app.post('/api/v1/internal/workflow/process-runs', { schema: { tags: ['Internal'] } }, async (req, reply) => {
  ...
  app.post('/api/v1/internal/workflow/send-email', { schema: { tags: ['Internal'] } }, async (req, reply) => {
    if (!checkSecret(req.headers['x-internal-secret'])) return reply.status(401).send();
    ...
    if (body.campaignId) {
      ...
      const from = await resolveOrgFrom(body.orgId);
      if (!from) return reply.send({ data: { skipped: true, reason: 'no verified sending domain' } });
      fromName = from.fromName;
  ...
  app.post('/api/v1/internal/workflow/send-sms', { schema: { tags: ['Internal'] } }, async (req, reply) => {
    if (!checkSecret(req.headers['x-internal-secret'])) return reply.status(401).send();
