...
import { z } from 'zod';
import {
  createCategory, listCategories, createGroup, listGroups,
  addContactToGroup, removeContactFromGroup, getContactGroups,
} from '../../services/groups/index.js';

const groupRoutes: FastifyPluginAsync = async (app) => {
  app.get('/api/v1/groups/categories', {
  ...
  app.post('/api/v1/groups/categories', {
  ...
  app.post('/api/v1/groups/:groupId/members/:contactId', {
    preHandler: [app.authenticate],
    schema: { tags: ['Groups'] },
  }, async (req, reply) => {
    const { groupId, contactId } = z.object({
      groupId: z.string().uuid(), contactId: z.string().uuid(),
    }).parse(req.params);
    await addContactToGroup(contactId, groupId, req.user!.orgId);
    return reply.code(204).send();
  });

  app.delete('/api/v1/groups/:groupId/members/:contactId', {
    preHandler: [app.authenticate],
    schema: { tags: ['Groups'] },
  }, async (req, reply) => {
    const { groupId, contactId } = z.object({
  ...
  app.get('/api/v1/contacts/:contactId/groups', {
    preHandler: [app.authenticate],
    schema: { tags: ['Groups'] },
  }, async (req, reply) => {
