...
const universalInboxRoutes: FastifyPluginAsync = async (app) => {
  app.post('/api/v1/helpdesk/inbox/ingest', {
  ...
  app.post('/api/v1/helpdesk/inbox/:ticketId/reply', {
    preHandler: [app.authenticate],
    schema: { tags: ['Helpdesk'] },
  }, async (req, reply) => {
  ...
  app.get('/api/v1/helpdesk/inbox/contact/:contactId', {
    preHandler: [app.authenticate],
    schema: { tags: ['Helpdesk'] },
  }, async (req, reply) => {
    ...
    return reply.send({ data: await getUnifiedContactHistory(req.user!.orgId, contactId, q.limit) });
  });
};
