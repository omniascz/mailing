...
import { z } from 'zod';
import {
  setAgentStatus, updateAgentProfile, listAgentAvailability,
  createRoutingRule, listRoutingRules, updateRoutingRule, deleteRoutingRule,
  autoAssignTicket, manualAssignTicket,
} from '../../../services/helpdesk/routing.js';

const helpdeskRoutingRoutes: FastifyPluginAsync = async (app) => {
  // ── Agent availability ──────────────────────────────────────────────────────

  app.get('/api/v1/helpdesk/agents', {
  ...
  app.patch('/api/v1/helpdesk/agents/me/status', {
    preHandler: [app.authenticate],
    schema: { tags: ['Helpdesk'] },
  }, async (req, reply) => {
  ...
  app.patch('/api/v1/helpdesk/agents/me/profile', {
    preHandler: [app.authenticate],
    schema: { tags: ['Helpdesk'] },
  }, async (req, reply) => {
    const body = z.object({
    ...
    return reply.send({ data: await updateAgentProfile(req.user!.orgId, req.user!.userId, body) });
  });

  // ── Ticket assignment ───────────────────────────────────────────────────────

  app.post('/api/v1/helpdesk/tickets/:id/assign', {
    preHandler: [app.authenticate],
    schema: { tags: ['Helpdesk'] },
  }, async (req, reply) => {
  ...
  app.post('/api/v1/helpdesk/tickets/:id/auto-assign', {
    preHandler: [app.authenticate],
    schema: { tags: ['Helpdesk'] },
  }, async (req, reply) => {
    const { id } = req.params as { id: string };
    ...
    const result = await autoAssignTicket(req.user!.orgId, id, body.channel ?? 'chat', body.requiredSkills);
    return reply.send({ data: result });
  });

  // ── Routing rules ───────────────────────────────────────────────────────────

  app.get('/api/v1/helpdesk/routing-rules', {
    preHandler: [app.authenticate],
    schema: { tags: ['Helpdesk'] },
  }, async (req, reply) => {
  ...
  app.post('/api/v1/helpdesk/routing-rules', {
  ...
  app.patch('/api/v1/helpdesk/routing-rules/:id', {
    preHandler: [app.authenticate, app.requireRole('admin')],
    schema: { tags: ['Helpdesk'] },
  }, async (req, reply) => {
    const { id } = req.params as { id: string };
    ...
    return reply.send({ data: await updateRoutingRule(req.user!.orgId, id, body as Parameters<typeof updateRoutingRule>[2]) });
  });

  app.delete('/api/v1/helpdesk/routing-rules/:id', {
    preHandler: [app.authenticate, app.requireRole('admin')],
    schema: { tags: ['Helpdesk'] },
  }, async (req, reply) => {
    const { id } = req.params as { id: string };
