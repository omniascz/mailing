...
import { computeUnsubscribeRisk, getAtRiskContacts } from '../../services/contacts/unsubscribe-prediction.js';
...
export default async function campaignFatigueRoutes(app: FastifyInstance) {
  ...
  app.post('/api/v1/campaign-fatigue/batch', async (req) => {
    const body = z.object({ contactIds: z.array(z.string().uuid()).min(1).max(200) }).parse(req.body);
