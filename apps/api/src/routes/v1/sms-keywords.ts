...
import { z } from 'zod';
import {
  createKeyword, listKeywords, deleteKeywordOrThrow, dispatchInboundSms,
} from '../../services/sms-keywords/index.js';

const smsKeywordRoutes: FastifyPluginAsync = async (app) => {
  app.get('/api/v1/sms/keywords', {
  ...
  app.delete('/api/v1/sms/keywords/:id', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner', 'editor')],
    schema: { tags: ['SMS'] },
  }, async (req, reply) => {
