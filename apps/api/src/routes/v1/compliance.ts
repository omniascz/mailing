...
const complianceRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post(
    '/api/v1/compliance/controls/seed',
    {
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      return reply.send({
        data: await runAccessReview(req.user!.orgId, req.user!.userId, body.findings),
      });
    },
  );
  ...
  app.post(
    ...
    async (req, reply) => {
      const body = z
        ...
        .parse(req.body);
      return reply.send({
        data: await setRetentionPolicy(req.user!.orgId, body.resource, body.retentionDays),
      });
    },
  );
