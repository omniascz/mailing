...
import { z } from 'zod';
import {
  createSurvey, listSurveys, getSurvey, updateSurvey,
...
const surveyRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/api/v1/surveys/:id/results', {
    preHandler: [app.authenticate],
    schema: { tags: ['Surveys'] },
  }, async (req, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
  ...
  app.post('/public/surveys/:id/submit', {
    schema: { tags: ['Surveys'] },
  }, async (req, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    const body = z.object({
