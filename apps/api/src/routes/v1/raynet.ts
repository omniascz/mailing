...
const raynetRoutes: FastifyPluginAsync = async (app) => {
  app.post('/api/v1/raynet/connect', {
  ...
  app.get('/api/v1/raynet/connection', {
  ...
  app.post('/api/v1/raynet/sync/contacts', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
  ...
  app.post('/api/v1/raynet/sync/companies', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
  ...
  app.post('/api/v1/raynet/sync/deals', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
  ...
  app.delete('/api/v1/raynet/connection', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
    schema: { tags: ['Raynet'], summary: 'Disconnect Raynet' },
  }, async (req, reply) => {
    await db
      .delete(raynetConnections)
      .where(eq(raynetConnections.orgId, req.user!.orgId));
