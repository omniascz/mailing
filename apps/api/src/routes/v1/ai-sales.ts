...
import {
  winProbability,
  winProbabilityForPipeline,
} from '../../services/ai-sales/win-probability.js';
...
const aiSalesRoutes: FastifyPluginAsync = async (app) => {
  app.get(
    '/api/v1/ai-sales/deals/:id/win-probability',
    {
  ...
  app.get(
    '/api/v1/ai-sales/pipelines/:id/win-probability',
    {
      preHandler: [app.authenticate],
      schema: { tags: ['AI Sales'] },
    },
  ...
  app.post(
    ...
    async (req, reply) => {
      const body = z
        ...
        .parse(req.body);
      return reply.send({
        data: await analyzeSentiment(req.user!.orgId, body.text, { forceAi: body.forceAi }),
      });
    },
  );

  app.get(
    '/api/v1/ai-sales/contacts/:id/predictive-score',
    {
      preHandler: [app.authenticate],
      schema: { tags: ['AI Sales'] },
    },
