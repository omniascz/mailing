...
import { unsubscribeExperiments, unsubscribeVariants } from '../../db/schema/unsubscribe-experiments.js';
import {
  assignVariant,
  recordImpression,
  recordOutcome,
...
export default async function unsubscribeAbRoutes(app: FastifyInstance) {
  ...
  app.get('/api/v1/unsubscribe-experiments', async (req) => {
    const rows = await db.select().from(unsubscribeExperiments)
  ...
  app.post('/api/v1/unsubscribe-experiments', async (req, reply) => {
    ...
    const [row] = await db.insert(unsubscribeExperiments)
  ...
  app.put('/api/v1/unsubscribe-experiments/:id', async (req) => {
    ...
    const [row] = await db.update(unsubscribeExperiments)
      ...
      .where(and(eq(unsubscribeExperiments.id, id), eq(unsubscribeExperiments.orgId, req.user!.orgId)))
      .returning();
    return { data: row };
  });

  const variantFlows = z.enum(['immediate_unsub', 'offer_pause', 'offer_downgrade', 'offer_preferences', 'offer_reason']);
  ...
  app.post('/api/v1/unsubscribe-experiments/:experimentId/variants', async (req, reply) => {
    ...
    const [row] = await db.insert(unsubscribeVariants)
  ...
  app.get('/api/v1/unsubscribe-experiments/:id/variants', async (req) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    const rows = await db.select().from(unsubscribeVariants)
      .where(and(eq(unsubscribeVariants.experimentId, id), eq(unsubscribeVariants.orgId, req.user!.orgId)));
    return { data: rows };
  });
