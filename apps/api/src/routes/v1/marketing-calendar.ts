...
const calendarRoutes: FastifyPluginAsync = async (app) => {
    ...
    async (req, reply) => {
      ...
      const entries: CalendarEntry[] = [
        ...
        ...socialPostRows.map((p): CalendarEntry => {
          ...
          const platforms = [
            ...new Set(accountIdList.map((id) => accountMap[id]?.platform).filter(Boolean)),
          ];
          return {
            type: 'social_post',
            id: p.id,
  ...
  app.patch(
    '/api/v1/marketing-calendar/:type/:id/reschedule',
    {
      preHandler: [app.authenticate],
      schema: { tags: ['Marketing Calendar'] },
    },
    async (req, reply) => {
      const { type, id } = z
        .object({ type: z.enum(['campaign', 'social_post']), id: z.string().uuid() })
        .parse(req.params);
