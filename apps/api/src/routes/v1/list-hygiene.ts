...
import { z } from 'zod';
import {
  generateHygieneReport, purgeInactive,
  findDuplicates, mergeDuplicates,
} from '../../services/list-hygiene/index.js';

const hygieneRoutes: FastifyPluginAsync = async (app) => {
  app.get('/api/v1/list-hygiene/report', {
  ...
  app.post('/api/v1/list-hygiene/purge', {
  ...
  app.post('/api/v1/duplicates/merge', {
    ...
    return reply.send({ data: await mergeDuplicates(req.user!.orgId, body.primaryId, body.duplicateIds) });
  });
};
