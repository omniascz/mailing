...
import {
  ...
  pushDealToHubSpot,
} from '../../../integrations/hubspot/sync.js';
...
const hubspotRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get(
    ...
    async (req, reply) => {
      ...
      return reply.redirect(
        `${process.env.APP_URL ?? 'http://localhost:3000'}/settings/integrations/hubspot?connected=1`,
      );
    },
  );
  ...
  app.get(
    ...
    async (req, reply) => {
      const [conn] = await db
        ...
        .where(eq(hubspotConnections.orgId, req.user!.orgId))
        .limit(1);
  ...
  app.post(
    '/api/v1/integrations/hubspot/sync/deals',
    {
      preHandler: [app.authenticate, app.requireRole('admin')],
      schema: { tags: ['HubSpot'] },
    },
  ...
  app.post(
    '/api/v1/integrations/hubspot/push/contact/:contactId',
    {
      preHandler: [app.authenticate, app.requireRole('admin')],
      schema: { tags: ['HubSpot'] },
    },
  ...
  app.post(
    '/api/v1/integrations/hubspot/push/deal/:dealId',
    {
      preHandler: [app.authenticate, app.requireRole('admin')],
      schema: { tags: ['HubSpot'] },
    },
