...
import { verifyWebhookSignature, processCalendlyEvent, type CalendlyInviteeEvent } from '../../../integrations/calendly/webhook.js';
import { AppError } from '../../../lib/app-error.js';

const calendlyRoutes: FastifyPluginAsync = async (app) => {
  app.get('/api/v1/integrations/calendly/connect', {
  ...
  app.get('/api/v1/integrations/calendly/callback', {
    schema: { tags: ['Calendly'] },
  }, async (req, reply) => {
    ...
    return reply.redirect(`${process.env.APP_URL ?? 'http://localhost:3000'}/settings/integrations/calendly?connected=1`);
  });

  app.get('/api/v1/integrations/calendly', {
    ...
    }).from(calendlyConnections).where(eq(calendlyConnections.orgId, req.user!.orgId)).limit(1);
    return reply.send({ data: conn ?? null });
  });

  app.delete('/api/v1/integrations/calendly', {
    preHandler: [app.authenticate, app.requireRole('admin')],
    schema: { tags: ['Calendly'] },
  }, async (req, reply) => {
  ...
  app.patch('/api/v1/integrations/calendly', {
    preHandler: [app.authenticate, app.requireRole('admin')],
    schema: { tags: ['Calendly'] },
  }, async (req, reply) => {
    const body = z.object({
    ...
    const [updated] = await db.update(calendlyConnections).set({ ...body, updatedAt: new Date() }).where(eq(calendlyConnections.orgId, req.user!.orgId)).returning();
  ...
  app.post('/api/v1/webhooks/calendly', {
    config: { rawBody: true },
    schema: { tags: ['Calendly'] },
  }, async (req, reply) => {
    ...
    const [conn] = await db.select({ webhookSigningKey: calendlyConnections.webhookSigningKey })
      .from(calendlyConnections).where(eq(calendlyConnections.orgId, orgId)).limit(1);

    if (conn?.webhookSigningKey && !verifyWebhookSignature(rawBody, signature, conn.webhookSigningKey)) {
