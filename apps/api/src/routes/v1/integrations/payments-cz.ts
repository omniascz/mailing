...
import {
  czPaymentGatewaySettings,
  czPaymentTransactions,
} from '../../../db/schema/payment-gateways-cz.js';
import { createGoPayPayment, verifyGoPayPayment } from '../../../services/integrations/gopay.js';
import {
  createComgatePayment,
  verifyComgatePayment,
} from '../../../services/integrations/comgate.js';
...
export default async function czPaymentRoutes(app: FastifyInstance) {
  ...
  app.put('/api/v1/integrations/payments-cz/settings', async (req) => {
    ...
    const existing = await db
      .select({ id: czPaymentGatewaySettings.id })
      .from(czPaymentGatewaySettings)
      .where(eq(czPaymentGatewaySettings.orgId, req.user!.orgId))
      .limit(1);
    ...
    if (existing.length) {
      [row] = await db
        .update(czPaymentGatewaySettings)
        .set(patch as never)
        .where(eq(czPaymentGatewaySettings.orgId, req.user!.orgId))
        .returning();
    } else {
      [row] = await db
        .insert(czPaymentGatewaySettings)
