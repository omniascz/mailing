...
async function getAllegroToken(orgId: string): Promise<string> {
  const [row] = await db
    ...
    .from(socialAccounts)
    .where(and(eq(socialAccounts.orgId, orgId), eq(socialAccounts.platform, 'allegro'), eq(socialAccounts.active, true)))
    .limit(1);
  if (!row?.accessToken) throw AppError.badRequest('Allegro account not connected. Use /api/v1/integrations/allegro/connect');
...
async function allegroFetch(orgId: string, path: string, opts: RequestInit = {}) {
  ...
  const resp = await fetch(`${ALLEGRO_API_BASE}${path}`, {
    ...opts,
    headers: { Authorization: `Bearer ${token}`, 'Accept': 'application/vnd.allegro.public.v1+json', 'Content-Type': 'application/json', ...(opts.headers ?? {}) },
  });
  if (!resp.ok) {
...
export default async function allegroRoutes(app: FastifyInstance) {
  ...
  app.get('/api/v1/integrations/allegro/status', async (req) => {
    const [row] = await db.select({ active: socialAccounts.active, updatedAt: socialAccounts.updatedAt })
      .from(socialAccounts)
  ...
  app.get('/api/v1/integrations/allegro/callback', async (req, reply) => {
    ...
    await db.insert(socialAccounts).values({
      ...
      active: true,
    }).onConflictDoUpdate({ target: [socialAccounts.orgId, socialAccounts.platform, socialAccounts.platformUserId], set: { accessToken: tokens['access_token']!, refreshToken: tokens['refresh_token'], active: true, updatedAt: new Date() } });

    await redis.del(`allegro:state:${state}`);
    return reply.redirect(`${process.env.WEB_URL ?? 'http://localhost:3000'}/settings/integrations?allegro=connected`);
  });

  // GET /api/v1/integrations/allegro/orders — fetch recent orders
  app.get('/api/v1/integrations/allegro/orders', async (req) => {
    const q = z.object({ limit: z.coerce.number().int().min(1).max(100).default(20), offset: z.coerce.number().int().min(0).default(0) }).parse(req.query);
    const data = await allegroFetch(req.user!.orgId, `/order/checkout-forms?limit=${q.limit}&offset=${q.offset}`);
    return { data };
  });
  ...
  app.post('/api/v1/integrations/allegro/sync-orders', async (req, reply) => {
    ...
    const orders = await allegroFetch(orgId, '/order/checkout-forms?limit=100') as { checkoutForms: Array<Record<string, unknown>> };
  ...
  app.get('/api/v1/integrations/allegro/products', async (req) => {
    const q = z.object({ phrase: z.string().optional(), limit: z.coerce.number().int().min(1).max(60).default(20) }).parse(req.query);
