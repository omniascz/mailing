...
export default async function crmActivityRoutes(app: FastifyInstance) {
  app.get(
    ...
    { preHandler: app.requireAuth, schema: { tags: ['CRM'], summary: 'Get contact activity feed' } },
    async (req) => {
      const { contactId } = contactIdParam.parse(req.params);
      const q = listQuery.parse(req.query);

      const types = q.types
        ? (q.types.split(',').filter((t) => ACTIVITY_TYPES.includes(t as ActivityItem['type'])) as ActivityItem['type'][])
        : undefined;
