...
import { z } from 'zod';
import {
  createAccount, getAccount, listAccounts, updateAccount, deleteAccount, listChildAccounts,
...
const accountRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/api/v1/crm/accounts/:id', {
    preHandler: [app.authenticate],
    schema: { tags: ['CRM Accounts'] },
  }, async (req, reply) => {
  ...
  app.patch('/api/v1/crm/accounts/:id', {
    preHandler: [app.authenticate, app.requireRole('owner', 'admin', 'editor')],
    schema: { tags: ['CRM Accounts'] },
  }, async (req, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
  ...
  app.delete('/api/v1/crm/accounts/:id', {
  ...
  app.get('/api/v1/crm/accounts/:id/children', {
    preHandler: [app.authenticate],
    schema: { tags: ['CRM Accounts'] },
  }, async (req, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
