...
import { z } from 'zod';
import {
  createDeal, getDeal, listDeals, updateDeal, moveDealToStage,
  markDealWon, markDealLost, deleteDeal, dealStageHistoryForDeal,
...
const dealRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/api/v1/crm/deals/:id/move-stage', {
    preHandler: [app.authenticate, app.requireRole('owner', 'admin', 'editor')],
    schema: { tags: ['CRM Deals'] },
  }, async (req, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    const { stageId } = z.object({ stageId: z.string().min(1) }).parse(req.body);
    return reply.send({ data: await moveDealToStage(req.user!.orgId, id, stageId, req.user!.userId) });
  });

  app.post('/api/v1/crm/deals/:id/won', {
    preHandler: [app.authenticate, app.requireRole('owner', 'admin', 'editor')],
    schema: { tags: ['CRM Deals'] },
  }, async (req, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    const body = z.object({ closeDate: isoDate.optional() }).parse(req.body ?? {});
    return reply.send({ data: await markDealWon(req.user!.orgId, id, body.closeDate, req.user!.userId) });
  });

  app.post('/api/v1/crm/deals/:id/lost', {
    preHandler: [app.authenticate, app.requireRole('owner', 'admin', 'editor')],
    schema: { tags: ['CRM Deals'] },
  }, async (req, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    ...
    return reply.send({ data: await markDealLost(req.user!.orgId, id, body.reason, body.closeDate, req.user!.userId) });
  });

  app.delete('/api/v1/crm/deals/:id', {
  ...
  app.get('/api/v1/crm/deals/:id/stage-history', {
    preHandler: [app.authenticate],
    schema: { tags: ['CRM Deals'] },
  }, async (req, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
