...
import { z } from 'zod';
import {
  createPipeline, listPipelines, getPipeline, updatePipeline, deletePipeline, seedDefaultPipeline,
...
const pipelineRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/api/v1/crm/pipelines/seed-default', {
    preHandler: [app.authenticate, app.requireRole('owner', 'admin')],
    schema: { tags: ['CRM Pipelines'] },
  }, async (req, reply) => {
  ...
  app.get('/api/v1/crm/pipelines/:id', {
    preHandler: [app.authenticate],
    schema: { tags: ['CRM Pipelines'] },
  }, async (req, reply) => {
  ...
  app.patch('/api/v1/crm/pipelines/:id', {
    preHandler: [app.authenticate, app.requireRole('owner', 'admin')],
    schema: { tags: ['CRM Pipelines'] },
  }, async (req, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
  ...
  app.delete('/api/v1/crm/pipelines/:id', {
    preHandler: [app.authenticate, app.requireRole('owner', 'admin')],
    schema: { tags: ['CRM Pipelines'] },
  }, async (req, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
