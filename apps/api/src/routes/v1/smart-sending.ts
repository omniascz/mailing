...
import { z } from 'zod';
import { listRules, upsertRule, canSend, recordSend, pruneSendLog } from '../../services/smart-sending/index.js';

const smartSendingRoutes: FastifyPluginAsync = async (app) => {
  app.get('/api/v1/smart-sending/rules', {
  ...
  app.put('/api/v1/smart-sending/rules', {
  ...
  app.get('/api/v1/smart-sending/check', {
    preHandler: [app.authenticate],
    schema: { tags: ['SmartSending'] },
  }, async (req, reply) => {
    const q = z.object({
      contactId: z.string().uuid(), channel: z.string(),
    }).parse(req.query);
    return reply.send({ data: await canSend(req.user!.orgId, q.contactId, q.channel) });
  });

  app.post('/api/v1/smart-sending/record', {
    preHandler: [app.authenticate],
    schema: { tags: ['SmartSending'] },
  }, async (req, reply) => {
    const body = z.object({
  ...
  app.post('/api/v1/smart-sending/prune', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
    schema: { tags: ['SmartSending'] },
  }, async (_req, reply) => {
