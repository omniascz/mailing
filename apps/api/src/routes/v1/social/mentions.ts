...
import {
  ...
  listMonitoringKeywords,
  removeMonitoringKeyword,
  listMentions,
  updateMentionStatus,
...
const socialMentionRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.patch('/api/v1/social/mentions/:mentionId', auth, async (req, reply) => {
    ...
    const { status } = z
      .object({ status: z.enum(['new', 'read', 'replied', 'ignored']) })
      .parse(req.body);
  ...
  app.post('/api/v1/social/mentions/:mentionId/to-inbox', auth, async (req, reply) => {
    ...
    const [mention] = await db
      .select()
      .from(socialMentions)
      .where(and(eq(socialMentions.orgId, req.user!.orgId), eq(socialMentions.id, mentionId)));
    if (!mention) return reply.code(404).send({ code: 'NOT_FOUND', message: 'Mention not found' });
    const result = await bridgeMentionToInbox(req.user!.orgId, {
      ...mention,
      rawData: mention.rawData as Record<string, unknown>,
    });
