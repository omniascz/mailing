...
const calcPropRoutes: FastifyPluginAsync = async (app) => {
  app.get('/api/v1/calculated-properties', {
  ...
  app.post('/api/v1/calculated-properties', {
    ...
    const body = z.object({
      ...
      cacheTtlSeconds: z.number().int().positive().max(30 * 24 * 3600).optional(),
    }).parse(req.body);
  ...
  app.delete('/api/v1/calculated-properties/:id', {
    preHandler: [app.authenticate, app.requireRole('admin')],
    schema: { tags: ['CalculatedProperties'] },
  }, async (req, reply) => {
  ...
  app.post('/api/v1/calculated-properties/preview', {
    preHandler: [app.authenticate],
    schema: { tags: ['CalculatedProperties'] },
  }, async (req, reply) => {
    const body = z.object({
