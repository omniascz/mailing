...
import { buildCampaign, type CampaignBuildInput } from '../../services/ai-agents/campaign-builder.js';
...
import {
  runSupportAgent,
  persistAgentDraft,
} from '../../services/ai-agents/customer-support.js';
...
export default async function aiAgentRoutes(app: FastifyInstance) {
  ...
  app.post('/api/v1/ai-agents/build-campaign', {
  ...
  // POST /api/v1/ai-agents/:id/run — trigger agent run
  app.post('/api/v1/ai-agents/:id/run', {
    schema: { tags: ['AI Agents'] },
  }, async (request, reply) => {
    const { orgId } = request as unknown as { orgId: string };
  ...
  app.get('/api/v1/ai-agents/deal-health/loss-analysis', {
  ...
  app.get('/api/v1/ai-agents/deal-health/scores', {
    preHandler: [app.authenticate],
    schema: { tags: ['AI Agents'] },
  }, async (request, reply) => {
    const { orgId } = request.user!;
  ...
  app.post('/api/v1/ai-agents/prospecting/find', {
    preHandler: [app.authenticate],
    schema: { tags: ['AI Agents'] },
  }, async (request, reply) => {
    const { orgId } = request.user!;
  ...
  app.get('/api/v1/ai-agents/prospecting/accounts/:id/research', {
    preHandler: [app.authenticate],
    schema: { tags: ['AI Agents'] },
  }, async (request, reply) => {
    const { orgId } = request.user!;
  ...
  app.post('/api/v1/ai-agents/prospecting/contacts/:id/outreach', {
    preHandler: [app.authenticate],
    schema: { tags: ['AI Agents'] },
  }, async (request, reply) => {
    const { orgId } = request.user!;
  ...
  app.post('/api/v1/ai-agents/support/tickets/:id/run', {
    preHandler: [app.authenticate],
    schema: { tags: ['AI Agents'] },
  }, async (request, reply) => {
    const { orgId } = request.user!;
  ...
  app.get('/api/v1/ai-agents/deal-health/stage-velocity', {
    preHandler: [app.authenticate],
    schema: { tags: ['AI Agents'] },
  }, async (request, reply) => {
    const { orgId } = request.user!;
  ...
  // GET /api/v1/ai-agents/:id/runs — list runs for agent
  app.get('/api/v1/ai-agents/:id/runs', {
    schema: { tags: ['AI Agents'] },
  }, async (request, reply) => {
    const { orgId } = request as unknown as { orgId: string };
    const { id } = request.params as { id: string };
    const runs = await db.select().from(aiAgentRuns)
