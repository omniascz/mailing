...
const calendarSyncRoutes: FastifyPluginAsync = async (app) => {
  app.get(
    '/api/v1/calendar/oauth/:provider/authorize',
    {
    ...
    async (req, reply) => {
      const { provider } = z
        ...
        .parse(req.params);
      const q = z
        .object({ redirectUri: z.string().url(), state: z.string().min(1) })
        .parse(req.query);
      const url =
        provider === 'google'
  ...
  app.get(
    ...
    async (req, reply) => {
      ...
      const rows = await listIntegrations(req.user!.orgId, q.mine ? req.user!.userId : undefined);
      return reply.send({ data: rows });
    },
  );

  app.delete(
    '/api/v1/calendar/integrations/:id',
    {
      preHandler: [app.authenticate],
      schema: { tags: ['Calendar'] },
    },
  ...
  app.post(
    '/api/v1/calendar/integrations/:id/sync',
    {
      preHandler: [app.authenticate],
      schema: { tags: ['Calendar'] },
    },
