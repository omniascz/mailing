...
const inboundEmailRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/api/v1/webhooks/inbound-email/:orgId', {
  ...
  app.post('/api/v1/webhooks/inbound-email/raw', {
    ...
    if (!payload.orgId) throw AppError.badRequest('orgId required (resolve recipient before posting)');
  ...
  app.get('/api/v1/inbound-emails/:id', {
    preHandler: [app.authenticate],
    schema: { tags: ['Inbound Email'] },
  }, async (req, reply) => {
