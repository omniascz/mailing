...
export default async function brandGuidelinesRoutes(app: FastifyInstance) {
  ...
  app.get('/api/v1/brand-guidelines', async (req) => {
    const rows = await db.select().from(brandGuidelines)
  ...
  app.post('/api/v1/brand-guidelines', async (req, reply) => {
    ...
    const existing = await db.select({ id: brandGuidelines.id }).from(brandGuidelines)
      .where(eq(brandGuidelines.orgId, req.user!.orgId));
    const [row] = await db.insert(brandGuidelines).values({
  ...
  app.put('/api/v1/brand-guidelines/:id', async (req) => {
    ...
    const [row] = await db.update(brandGuidelines)
      .set(patch as never)
  ...
  app.post('/api/v1/brand-guidelines/:id/activate', async (req) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    await db.update(brandGuidelines).set({ active: false }).where(eq(brandGuidelines.orgId, req.user!.orgId));
    const [row] = await db.update(brandGuidelines)
  ...
  app.post('/api/v1/brand-guidelines/check', async (req) => {
    ...
    const report = await checkBrandConsistency(req.user!.orgId, body.subject, body.html, body.guidelineId);
    return { data: report };
  });
}
