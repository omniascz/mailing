...
const sendOptimizationRoutes: FastifyPluginAsync = async (app) => {
  app.post('/api/v1/send-optimization/record-open', {
  ...
  app.get('/api/v1/send-optimization/best-hour/:contactId', {
    preHandler: [app.authenticate],
    schema: { tags: ['Send Optimization'] },
  }, async (req, reply) => {
  ...
  app.get('/api/v1/send-optimization/best-day', {
    preHandler: [app.authenticate],
    schema: { tags: ['Send Optimization'] },
  }, async (req, reply) => {
  ...
  app.post('/api/v1/send-optimization/timewarp', {
    preHandler: [app.authenticate],
    schema: { tags: ['Send Optimization'] },
  }, async (req, reply) => {
    const body = z.object({
  ...
  app.post('/api/v1/send-optimization/backfill-timezones', {
