...
const dataSyncRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.patch(
    ...
    async (req, reply) => {
      ...
      await db
        .update(dataSyncMappings)
        .set({ ...body, updatedAt: new Date() })
        .where(and(eq(dataSyncMappings.orgId, req.user!.orgId), eq(dataSyncMappings.id, id)));
  ...
  app.get(
    ...
    async (req, reply) => {
      const rows = await db
        .select()
        .from(dataSyncConflicts)
        .where(
          and(eq(dataSyncConflicts.orgId, req.user!.orgId), eq(dataSyncConflicts.resolved, false)),
        );
      return reply.send({ data: rows });
    },
  );
  ...
  app.post(
    '/api/v1/data-sync/conflicts/:id/resolve',
    {
      preHandler: [app.authenticate, app.requireRole('admin')],
      schema: { tags: ['DataSync'] },
    },
