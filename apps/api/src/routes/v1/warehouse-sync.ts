...
import { z } from 'zod';
import {
  createWarehouseSync, listWarehouseSyncs,
  deleteWarehouseSync, runSync,
} from '../../services/warehouse-sync/index.js';

const warehouseSyncRoutes: FastifyPluginAsync = async (app) => {
  app.get('/api/v1/warehouse-syncs', {
  ...
  app.post('/api/v1/warehouse-syncs', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
    schema: { tags: ['Warehouse'] },
  }, async (req, reply) => {
    const body = z.object({
      ...
      entities: z.array(z.enum(['contacts', 'email_events', 'revenue_events', 'campaigns', 'workflow_runs'])).min(1),
  ...
  app.delete('/api/v1/warehouse-syncs/:id', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
    schema: { tags: ['Warehouse'] },
  }, async (req, reply) => {
  ...
  app.post('/api/v1/warehouse-syncs/:id/run', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
    schema: { tags: ['Warehouse'] },
  }, async (req, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
