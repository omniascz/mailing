...
export default async function inboxPlacementRoutes(app: FastifyInstance) {
  ...
  app.get('/api/v1/inbox-placement', async (req) => {
    const q = z.object({ limit: z.coerce.number().int().min(1).max(50).default(20) }).parse(req.query);
    const rows = await db.select().from(inboxPlacementTests)
  ...
  app.get('/api/v1/inbox-placement/:id', async (req) => {
    ...
    const [row] = await db.select().from(inboxPlacementTests)
