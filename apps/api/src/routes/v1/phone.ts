...
const phoneRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/api/v1/phone/calls/:id/hangup', {
    preHandler: [app.authenticate],
    schema: { tags: ['Phone'] },
  }, async (req, reply) => {
    const { id } = z.object({ id: z.string().min(1) }).parse(req.params);
    const body = z.object({ provider: z.enum(['twilio', 'telnyx']).optional() }).parse(req.body ?? {});
    await terminateCall(id, body.provider);
    return reply.code(204).send();
  });

  app.post('/api/v1/phone/webhook/:provider', {
  ...
  // ─── Routing admin (#251) ──────────────────────────────────────────────────
  app.get('/api/v1/phone/hunt-groups', {
    preHandler: [app.authenticate],
    schema: { tags: ['Phone'] },
  }, async (req, reply) => {
  ...
  app.post('/api/v1/phone/hunt-groups', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
    schema: { tags: ['Phone'] },
  }, async (req, reply) => {
    const body = z.object({
      name: z.string().min(1).max(255),
      strategy: z.enum(['ring-all', 'round-robin', 'least-idle', 'priority']).default('ring-all'),
  ...
  app.patch('/api/v1/phone/hunt-groups/:id', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
    schema: { tags: ['Phone'] },
  }, async (req, reply) => {
  ...
  app.delete('/api/v1/phone/hunt-groups/:id', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
    schema: { tags: ['Phone'] },
  }, async (req, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
  ...
  app.get('/api/v1/phone/ivr-menus', {
    preHandler: [app.authenticate],
    schema: { tags: ['Phone'] },
  }, async (req, reply) => {
  ...
  app.post('/api/v1/phone/ivr-menus', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
    schema: { tags: ['Phone'] },
  }, async (req, reply) => {
    const body = z.object({
      ...
      options: z.array(z.object({
        ...
        action: z.enum(['hunt-group', 'ivr-menu', 'voicemail', 'voicebot', 'hangup', 'external']),
  ...
  app.patch('/api/v1/phone/ivr-menus/:id', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
    schema: { tags: ['Phone'] },
  }, async (req, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
  ...
  app.delete('/api/v1/phone/ivr-menus/:id', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
    schema: { tags: ['Phone'] },
  }, async (req, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
  ...
  app.get('/api/v1/phone/business-hours', {
    preHandler: [app.authenticate],
    schema: { tags: ['Phone'] },
  }, async (req, reply) => {
  ...
  app.put('/api/v1/phone/business-hours', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
    schema: { tags: ['Phone'] },
  }, async (req, reply) => {
    const body = z.object({
      ...
      afterHoursTarget: z.enum(['voicemail', 'voicebot', 'external', 'hangup']).default('voicemail'),
