...
const meetingRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get(
    ...
    async (req, reply) => {
      ...
      const [page] = await db
        .select()
        .from(bookingPages)
        .where(eq(bookingPages.slug, pageSlug))
        .limit(1);
      ...
      const types = await db
        .select()
        .from(eventTypes)
        .where(and(eq(eventTypes.ownerUserId, page.ownerUserId), eq(eventTypes.isActive, true)));
  ...
  app.get(
    '/meetings/:pageSlug/:eventSlug/slots',
    {
      schema: { tags: ['Meetings (Public)'] },
    },
    async (req, reply) => {
  ...
  app.post(
    '/meetings/:pageSlug/:eventSlug/book',
    {
      schema: { tags: ['Meetings (Public)'] },
    },
    async (req, reply) => {
  ...
  app.patch(
    '/api/v1/meetings/event-types/:id',
    {
      preHandler: [app.authenticate],
      schema: { tags: ['Meetings'] },
    },
  ...
  app.patch(
    '/api/v1/meetings/bookings/:id/cancel',
    {
      preHandler: [app.authenticate],
      schema: { tags: ['Meetings'] },
    },
  ...
  app.patch(
    '/api/v1/meetings/bookings/:id/reschedule',
    {
      preHandler: [app.authenticate],
      schema: { tags: ['Meetings'] },
    },
