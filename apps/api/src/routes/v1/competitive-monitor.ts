...
export default async function competitiveMonitorRoutes(app: FastifyInstance) {
  ...
  app.post('/api/v1/competitive-monitor', async (req, reply) => {
    ...
    return reply.status(201).send({
      data: row,
      instruction: `Subscribe to ${body.competitorName} using ${monitorEmail}`,
    });
  });
  ...
  app.get('/api/v1/competitive-monitor/:watchlistId/emails', async (req) => {
    ...
    const rows = await db
      ...
      .where(
        and(
          eq(competitorEmails.orgId, req.user!.orgId),
          eq(competitorEmails.watchlistId, watchlistId),
        ),
      )
  ...
  app.post(
    '/api/v1/competitive-monitor/ingest',
    ...
    async (req, reply) => {
      ...
      const [watchEntry] = await db
        ...
        .where(
          and(
            eq(competitorWatchlist.monitorEmail, body.monitorEmail),
            eq(competitorWatchlist.active, true),
          ),
        )
        .limit(1);
  ...
  app.get('/api/v1/competitive-monitor/:watchlistId/analytics', async (req) => {
    ...
    const [entry] = await db
      ...
      .where(
        and(
          eq(competitorWatchlist.id, watchlistId),
          eq(competitorWatchlist.orgId, req.user!.orgId),
        ),
      )
      .limit(1);
    ...
    const emails = await db
      ...
      .from(competitorEmails)
      .where(
        and(eq(competitorEmails.watchlistId, watchlistId), gte(competitorEmails.receivedAt, d90)),
      )
      .orderBy(desc(competitorEmails.receivedAt));
    ...
    const allTopics = emails.flatMap(
      (e) => (e.analysis as { topics?: string[] } | null)?.topics ?? [],
    );
    ...
    const promotions = emails.filter(
      (e) => (e.analysis as { hasPromotion?: boolean } | null)?.hasPromotion,
    ).length;
    ...
    return {
      data: {
        ...
        topTopics,
        recentSubjects: emails
          .slice(0, 10)
          .map((e) => ({ subject: e.subjectLine, receivedAt: e.receivedAt })),
      },
    };
  });
}
