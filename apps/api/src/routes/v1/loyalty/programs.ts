...
const loyaltyProgramRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/api/v1/loyalty/programs', {
    ...
    const rows = await db
      ...
      .where(
        and(eq(loyaltyPrograms.orgId, req.user!.orgId), isNull(loyaltyPrograms.deletedAt)),
      )
      .orderBy(loyaltyPrograms.createdAt);
    return { data: rows };
  });

  app.post('/api/v1/loyalty/programs', {
    preHandler: [app.authenticate],
  ...
  app.get('/api/v1/loyalty/programs/:id', {
    preHandler: [app.authenticate],
    ...
    const [row] = await db
      ...
      .from(loyaltyPrograms)
      .where(
        and(eq(loyaltyPrograms.id, id), eq(loyaltyPrograms.orgId, req.user!.orgId), isNull(loyaltyPrograms.deletedAt)),
      )
      .limit(1);
  ...
  app.patch('/api/v1/loyalty/programs/:id', {
    preHandler: [app.authenticate],
    ...
    const [updated] = await db
      ...
      .where(
        and(eq(loyaltyPrograms.id, id), eq(loyaltyPrograms.orgId, req.user!.orgId)),
      )
      .returning();
    if (!updated) return reply.status(404).send({ code: 'NOT_FOUND', message: 'Program not found' });
    return { data: updated };
  });

  app.delete('/api/v1/loyalty/programs/:id', {
    preHandler: [app.authenticate],
  ...
  app.post('/api/v1/loyalty/programs/:id/members/enroll', {
    preHandler: [app.authenticate],
  ...
  app.get('/api/v1/loyalty/programs/:id/members', {
    preHandler: [app.authenticate],
  ...
  app.get('/api/v1/loyalty/programs/:id/members/:memberId', {
    preHandler: [app.authenticate],
    schema: { tags: ['Loyalty'], summary: 'Get a loyalty member' },
  }, async (req, reply) => {
    const { id: _programId, memberId } = z.object({
      id: z.string().uuid(), memberId: z.string().uuid(),
    }).parse(req.params);
    const member = await getMember(req.user!.orgId, memberId);
    if (!member) return reply.status(404).send({ code: 'NOT_FOUND', message: 'Member not found' });
    return { data: member };
  });

  app.patch('/api/v1/loyalty/programs/:id/members/:memberId/status', {
    preHandler: [app.authenticate],
    schema: { tags: ['Loyalty'], summary: 'Update member status' },
  }, async (req) => {
    const { memberId } = z.object({ id: z.string().uuid(), memberId: z.string().uuid() }).parse(req.params);
    const { status } = z.object({ status: z.enum(['active', 'suspended', 'opted_out']) }).parse(req.body);
    const member = await updateMemberStatus(req.user!.orgId, memberId, status);
    return { data: member };
  });

  app.post('/api/v1/loyalty/programs/:id/members/:memberId/credit', {
    preHandler: [app.authenticate],
