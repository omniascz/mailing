...
const loyaltyEarningRuleRoutes: FastifyPluginAsync = async (app) => {
  app.get('/api/v1/loyalty/programs/:programId/earning-rules', {
  ...
  app.post('/api/v1/loyalty/programs/:programId/earning-rules', {
    preHandler: [app.authenticate],
    ...
    const body = z.object({
      ...
      eventType: z.enum(['purchase', 'signup', 'birthday', 'referral', 'review', 'social_share', 'profile_complete', 'custom_event']),
  ...
  app.patch('/api/v1/loyalty/programs/:programId/earning-rules/:ruleId', {
    preHandler: [app.authenticate],
    schema: { tags: ['Loyalty'], summary: 'Update an earning rule' },
  }, async (req) => {
    const { ruleId } = z.object({
      programId: z.string().uuid(), ruleId: z.string().uuid(),
    }).parse(req.params);
    const body = z.object({
  ...
  app.delete('/api/v1/loyalty/programs/:programId/earning-rules/:ruleId', {
    preHandler: [app.authenticate],
  ...
  app.post('/api/v1/loyalty/programs/:programId/earn', {
    preHandler: [app.authenticate],
