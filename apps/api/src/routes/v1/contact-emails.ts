...
import { z } from 'zod';
import {
  addEmail, listEmails, setPrimary, setConsent,
  verifyEmail, removeEmail, bestSendableEmail,
} from '../../services/multi-email/index.js';

const contactEmailRoutes: FastifyPluginAsync = async (app) => {
  app.get('/api/v1/contacts/:contactId/emails', {
  ...
  app.post('/api/v1/contacts/:contactId/emails', {
    preHandler: [app.authenticate],
    schema: { tags: ['Contacts'] },
  }, async (req, reply) => {
    const { contactId } = z.object({ contactId: z.string().uuid() }).parse(req.params);
  ...
  app.patch('/api/v1/contacts/:contactId/emails/:emailId/primary', {
    preHandler: [app.authenticate],
    schema: { tags: ['Contacts'] },
  }, async (req, reply) => {
    const p = z.object({ contactId: z.string().uuid(), emailId: z.string().uuid() }).parse(req.params);
    return reply.send({ data: await setPrimary(p.contactId, p.emailId) });
  });

  app.patch('/api/v1/contacts/:contactId/emails/:emailId/consent', {
    preHandler: [app.authenticate],
    schema: { tags: ['Contacts'] },
  }, async (req, reply) => {
    const p = z.object({ contactId: z.string().uuid(), emailId: z.string().uuid() }).parse(req.params);
  ...
  app.post('/api/v1/contacts/:contactId/emails/:emailId/verify', {
    preHandler: [app.authenticate],
    schema: { tags: ['Contacts'] },
  }, async (req, reply) => {
    const p = z.object({ contactId: z.string().uuid(), emailId: z.string().uuid() }).parse(req.params);
  ...
  app.delete('/api/v1/contacts/:contactId/emails/:emailId', {
    preHandler: [app.authenticate],
    schema: { tags: ['Contacts'] },
  }, async (req, reply) => {
    const p = z.object({ contactId: z.string().uuid(), emailId: z.string().uuid() }).parse(req.params);
  ...
  app.get('/api/v1/contacts/:contactId/emails/best', {
    preHandler: [app.authenticate],
    schema: { tags: ['Contacts'] },
  }, async (req, reply) => {
    const { contactId } = z.object({ contactId: z.string().uuid() }).parse(req.params);
