...
const sklikLookalikeRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/api/v1/ads/sklik/lookalike', adminAuth, async (req, reply) => {
    const body = z
      .object({
    ...
    if (account.platform !== 'sklik') {
      return reply
        .code(422)
        .send({ code: 'WRONG_PLATFORM', message: 'Account is not a Sklik account' });
    }
  ...
  app.get('/api/v1/ads/sklik/conversions', adminAuth, async (req, reply) => {
  ...
  app.post('/api/v1/ads/sklik/conversions/report', adminAuth, async (req, reply) => {
    const body = z
      .object({
        adAccountId: z.string().uuid(),
    ...
    await reportSklikConversion(account.accessToken ?? '', account.platformAccountId, {
      goalId: body.goalId,
