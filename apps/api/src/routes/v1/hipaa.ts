...
const hipaaRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/api/v1/hipaa/mode/enable', {
  ...
  app.post('/api/v1/hipaa/mode/disable', {
    preHandler: [app.authenticate, app.requireRole('owner')],
  ...
  app.patch('/api/v1/hipaa/baa/:id', {
    ...
    const body = z.object({
      ...
      signedAt: z.string().datetime().optional().transform((v) => v ? new Date(v) : undefined),
      notes: z.string().optional(),
    }).parse(req.body);
    return { data: await updateBaa(req.user!.orgId, id, body) };
  });

  app.delete('/api/v1/hipaa/baa/:id', {
    preHandler: [app.authenticate, app.requireRole('owner')],
  ...
  app.get('/api/v1/hipaa/phi-fields', {
    preHandler: [app.authenticate, app.requireRole('owner', 'admin')],
  ...
  app.post('/api/v1/hipaa/phi-fields', {
    preHandler: [app.authenticate, app.requireRole('owner', 'admin')],
  ...
  app.delete('/api/v1/hipaa/phi-fields/:fieldKey', {
    preHandler: [app.authenticate, app.requireRole('owner', 'admin')],
  ...
  app.get('/api/v1/hipaa/access-log', {
    preHandler: [app.authenticate, app.requireRole('owner', 'admin')],
