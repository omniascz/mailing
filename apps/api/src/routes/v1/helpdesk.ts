...
import { z } from 'zod';
import {
  openTicket, listTickets, getTicket, appendMessage,
  updateStatus, assignTicket, hasOpenTicket,
...
const helpdeskRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/api/v1/helpdesk/tickets/:id', {
    preHandler: [app.authenticate],
    schema: { tags: ['Helpdesk'] },
  }, async (req, reply) => {
  ...
  app.post('/api/v1/helpdesk/tickets/:id/messages', {
    preHandler: [app.authenticate],
    schema: { tags: ['Helpdesk'] },
  }, async (req, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
  ...
  app.patch('/api/v1/helpdesk/tickets/:id/status', {
    preHandler: [app.authenticate],
    schema: { tags: ['Helpdesk'] },
  }, async (req, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
  ...
  app.patch('/api/v1/helpdesk/tickets/:id/assign', {
  ...
  app.get('/api/v1/helpdesk/has-open/:contactId', {
    preHandler: [app.authenticate],
    schema: { tags: ['Helpdesk'] },
  }, async (req, reply) => {
