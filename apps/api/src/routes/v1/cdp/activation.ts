...
const DestinationKindEnum = z.enum([
  'webhook', 'mailchimp', 'salesforce', 'hubspot',
...
const activationRoutes: FastifyPluginAsync = async (app) => {
  app.post('/api/v1/cdp/activation/destinations', {
  ...
  app.get('/api/v1/cdp/activation/destinations', {
  ...
  app.delete('/api/v1/cdp/activation/destinations/:id', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
    schema: { tags: ['CDP'] },
  }, async (req, reply) => {
  ...
  app.post('/api/v1/cdp/activation/destinations/:id/run', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
    schema: { tags: ['CDP'] },
  }, async (req, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
  ...
  app.get('/api/v1/cdp/activation/destinations/:id/runs', {
    preHandler: [app.authenticate],
    schema: { tags: ['CDP'] },
  }, async (req, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
