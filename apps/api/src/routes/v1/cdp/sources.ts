...
const cdpSourceRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/api/v1/cdp/sources', {
    preHandler: [app.authenticate, app.requireRole('admin')],
    schema: { tags: ['CDP Sources'] },
  }, async (req, reply) => {
    const body = sourceSchema.parse(req.body);
    const [row] = await db.insert(cdpSources).values({
  ...
  app.patch('/api/v1/cdp/sources/:id', {
    preHandler: [app.authenticate, app.requireRole('admin')],
    schema: { tags: ['CDP Sources'] },
  }, async (req, reply) => {
    const { id } = req.params as { id: string };
    const body = sourceSchema.partial().parse(req.body);
    const [row] = await db.update(cdpSources)
  ...
  app.delete('/api/v1/cdp/sources/:id', {
    preHandler: [app.authenticate, app.requireRole('admin')],
    schema: { tags: ['CDP Sources'] },
  }, async (req, reply) => {
    const { id } = req.params as { id: string };
    const [row] = await db.delete(cdpSources)
      .where(and(eq(cdpSources.id, id), eq(cdpSources.orgId, req.user!.orgId)))
  ...
  app.post('/api/v1/cdp/sources/:id/sync', {
    preHandler: [app.authenticate, app.requireRole('admin')],
    schema: { tags: ['CDP Sources'] },
  }, async (req, reply) => {
    const { id } = req.params as { id: string };
    const [source] = await db.select({ id: cdpSources.id, orgId: cdpSources.orgId })
      .from(cdpSources)
      .where(and(eq(cdpSources.id, id), eq(cdpSources.orgId, req.user!.orgId))).limit(1);
  ...
  app.get('/api/v1/cdp/sources/:id/runs', {
    preHandler: [app.authenticate],
    schema: { tags: ['CDP Sources'] },
  }, async (req, reply) => {
    const { id } = req.params as { id: string };
    ...
    const rows = await db.select().from(cdpSyncRuns)
  ...
  app.post('/api/v1/cdp/sources/:id/webhook', {
    schema: { tags: ['CDP Sources'] },
  }, async (req, reply) => {
    const { id } = req.params as { id: string };
    const [source] = await db.select().from(cdpSources)
      .where(eq(cdpSources.id, id)).limit(1);
    ...
    await db.update(cdpSources).set({ lastSyncAt: new Date(), updatedAt: new Date() })
