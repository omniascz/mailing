...
import {
  getBalance, addCredits, getTransactionHistory,
} from '../../services/billing/credits.js';
import {
  getProductUsage, getOrgUsageSummary, computePeriodCost, currentPeriod,
...
const billingExtendedRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/api/v1/billing/plans/all', {
    schema: { tags: ['Billing'] },
  }, async (_req, reply) => {
  ...
  app.get('/api/v1/billing/credits/history', {
    preHandler: [app.authenticate],
    schema: { tags: ['Billing'] },
  }, async (req, reply) => {
  ...
  app.get('/api/v1/billing/usage', {
    ...
    return reply.send({ data: { period: period ?? currentPeriod(), products: summary, totalCostUsd: totalCost } });
  });

  app.get('/api/v1/billing/usage/:product', {
    preHandler: [app.authenticate],
    schema: { tags: ['Billing'] },
  }, async (req, reply) => {
  ...
  app.get('/api/v1/billing/subaccounts/report', {
    preHandler: [app.authenticate, app.requireRole('owner')],
    schema: { tags: ['Billing'] },
  }, async (req, reply) => {
