...
const customChannelRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/api/v1/channels/custom', {
  ...
  app.post('/api/v1/channels/custom', {
  ...
  app.post('/api/v1/channels/custom/:id/rotate-secret', {
    preHandler: [app.authenticate, app.requireRole('admin')],
    schema: { tags: ['CustomChannels'] },
  }, async (req, reply) => {
  ...
  app.post('/api/v1/channels/custom/:id/disable', {
    preHandler: [app.authenticate, app.requireRole('admin')],
    schema: { tags: ['CustomChannels'] },
  }, async (req, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
  ...
  app.delete('/api/v1/channels/custom/:id', {
    preHandler: [app.authenticate, app.requireRole('admin')],
    schema: { tags: ['CustomChannels'] },
  }, async (req, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
  ...
  app.post('/api/v1/channels/custom/:slug/test', {
    preHandler: [app.authenticate, app.requireRole('admin')],
    schema: { tags: ['CustomChannels'] },
  }, async (req, reply) => {
  ...
  app.post('/api/v1/channels/custom/:orgId/:slug/inbound', {
    schema: { tags: ['CustomChannels'] },
  }, async (req, reply) => {
    const { orgId, slug } = z.object({
      orgId: z.string().uuid(),
  ...
  app.get('/api/v1/channels/custom/:slug', {
    preHandler: [app.authenticate],
    schema: { tags: ['CustomChannels'] },
  }, async (req, reply) => {
    const { slug } = z.object({ slug: z.string().min(1).max(64) }).parse(req.params);
