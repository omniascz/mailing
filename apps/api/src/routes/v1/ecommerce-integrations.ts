...
const ecommerceRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/api/v1/ecommerce/connections', {
  ...
  app.get('/api/v1/ecommerce/connections/:id', {
    preHandler: [app.authenticate],
  ...
  app.delete('/api/v1/ecommerce/connections/:id', {
  ...
  app.post('/api/v1/ecommerce/connections/:id/test', {
    preHandler: [app.authenticate],
    ...
    if (!result.ok) {
      await db
        ...
        .set({ status: 'error', lastErrorAt: new Date(), lastErrorMessage: result.error ?? 'Test failed' })
  ...
  app.post('/api/v1/ecommerce/connections', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
    schema: { tags: ['E-commerce'], summary: 'Create WooCommerce / BigCommerce / Magento / PrestaShop connection' },
  }, async (req, reply) => {
    ...
    const conn = await createConnection(req.user!.orgId, {
      platform,
      name,
      credentials: rest as WooCommerceCredentials | BigCommerceCredentials | MagentoCredentials | PrestaShopCredentials,
    });
  ...
  app.get('/api/v1/ecommerce/shopify/install', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
    schema: { tags: ['E-commerce'], summary: 'Start Shopify OAuth install flow' },
  }, async (req, reply) => {
    const { shop, name } = z.object({
    ...
    await redis.set(stateKey, JSON.stringify({ orgId: req.user!.orgId, shop, name: name ?? shop }), 'EX', 600);

    const installUrl = buildShopifyInstallUrl(shop, state);
    return reply.redirect(installUrl);
  });

  app.get('/api/v1/ecommerce/shopify/callback', {
    schema: { tags: ['E-commerce'], summary: 'Shopify OAuth callback' },
  }, async (req, reply) => {
    const { code, shop, state } = z.object({
      code: z.string(),
      shop: z.string(),
      state: z.string(),
    }).parse(req.query);
    ...
    if (!stateRaw) {
      return reply.code(400).send({ code: 'INVALID_STATE', message: 'OAuth state expired or invalid' });
    }
    ...
    // Save connection
    const credentials: ShopifyCredentials = { shopDomain: shop, accessToken, scopes: scope.split(',') };
  ...
  app.get('/api/v1/ecommerce/shoptet/install', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
    schema: { tags: ['E-commerce'], summary: 'Start Shoptet OAuth install flow' },
  }, async (req, reply) => {
    const { eshop, name } = z.object({
  ...
  app.get('/api/v1/ecommerce/shoptet/callback', {
  ...
  app.post('/api/v1/ecommerce/fastcentrik/install', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
  ...
  app.post('/api/v1/ecommerce/fastcentrik/:connectionId/pull', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
    ...
    if (conn.platform !== 'fastcentrik') {
      return reply.code(400).send({ code: 'WRONG_PLATFORM', message: 'Not a FastCentrik connection' });
    }
  ...
  app.post('/api/v1/ecommerce/upgates/install', {
    preHandler: [app.authenticate, app.requireRole('admin', 'owner')],
    ...
    if (!ping.ok) {
      return reply.code(400).send({ code: 'UPGATES_AUTH_FAILED', message: `Upgates API returned ${ping.status}` });
    }
  ...
  // Shopify webhook
  app.post('/api/v1/ecommerce/webhooks/shopify', {
    ...
    if (!shopDomain || !hmac) {
      return reply.code(400).send({ code: 'MISSING_HEADERS', message: 'Shopify headers missing' });
    }
  ...
  // Upgates webhook (#390)
  app.post('/api/v1/ecommerce/webhooks/upgates/:connectionId', {
    ...
    const [conn] = await db
      .select()
      .from(ecommerceConnections)
      .where(and(eq(ecommerceConnections.id, connectionId), eq(ecommerceConnections.platform, 'upgates')))
      .limit(1);

    if (!conn) return reply.code(204).send();

    const creds = conn.credentials as UpgatesCredentials;
    if (creds.webhookSecret && !verifyUpgatesWebhookSignature(rawBody, signature, creds.webhookSecret)) {
      return reply.code(401).send({ code: 'INVALID_SIGNATURE', message: 'HMAC mismatch' });
    }
  ...
  // Shoptet webhook (#386)
  app.post('/api/v1/ecommerce/webhooks/shoptet/:connectionId', {
    ...
    const [conn] = await db
      .select()
      .from(ecommerceConnections)
      .where(and(eq(ecommerceConnections.id, connectionId), eq(ecommerceConnections.platform, 'shoptet')))
      .limit(1);
  ...
  // WooCommerce webhook
  app.post('/api/v1/ecommerce/webhooks/woocommerce/:connectionId', {
    ...
    if (creds.webhookSecret && !verifyWooCommerceWebhook(rawBody, signature, creds.webhookSecret)) {
      return reply.code(401).send({ code: 'INVALID_SIGNATURE', message: 'HMAC mismatch' });
    }
  ...
  // BigCommerce webhook
  app.post('/api/v1/ecommerce/webhooks/bigcommerce/:connectionId', {
    ...
    if (creds.webhookSecret && !verifyBigCommerceWebhook(rawBody, signature, creds.webhookSecret)) {
      return reply.code(401).send({ code: 'INVALID_SIGNATURE', message: 'HMAC mismatch' });
    }
  ...
  app.post('/api/v1/ecommerce/webhooks/generic/:connectionId', {
    ...
    if (credAny['webhookSecret']) {
      ...
      if (secret !== expected) {
        return reply.code(401).send({ code: 'INVALID_SECRET', message: 'Webhook secret mismatch' });
      }
    }
