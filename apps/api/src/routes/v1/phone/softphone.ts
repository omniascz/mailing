...
const agentSockets = new Map<string, Set<import('@fastify/websocket').WebSocket>>();
...
const softphoneRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/api/v1/phone/softphone/token', {
  ...
  app.post('/api/v1/phone/softphone/answer', {
    preHandler: [app.authenticate],
    schema: { tags: ['Softphone'] },
  }, async (req, reply) => {
    const { callId } = z.object({ callId: z.string().uuid() }).parse(req.body);
    const [call] = await db.select().from(calls)
      .where(and(eq(calls.id, callId), eq(calls.orgId, req.user!.orgId))).limit(1);
  ...
  app.post('/api/v1/phone/softphone/hangup', {
    preHandler: [app.authenticate],
    schema: { tags: ['Softphone'] },
  }, async (req, reply) => {
    const { callId } = z.object({ callId: z.string().uuid() }).parse(req.body);
  ...
  app.post('/api/v1/phone/twiml/outbound', {
    schema: { tags: ['Softphone'] },
  }, async (req, reply) => {
...
<Response>
  ...
  app.post('/api/v1/phone/voicemail/recorded', {
    schema: { tags: ['Softphone'] },
  }, async (req, reply) => {
    const body = z.object({
      CallSid: z.string(),
  ...
  app.post('/api/v1/webhooks/twilio/call-status', {
    schema: { tags: ['Softphone'] },
  }, async (req, reply) => {
    const body = z.object({
      CallSid: z.string(),
    ...
    const [call] = await db.select().from(calls)
      .where(eq(calls.twilioCallSid, body.CallSid)).limit(1);
  ...
  app.post('/api/v1/webhooks/twilio/recording', {
    schema: { tags: ['Softphone'] },
  }, async (req, reply) => {
    const body = z.object({
      CallSid: z.string(),
  ...
  app.get('/api/v1/phone/softphone/ws', { websocket: true }, (socket, req) => {
    ...
    if (!agentId) { socket.close(4001, 'Unauthorized'); return; }
...
export function notifyAgentOfInboundCall(agentId: string, callId: string, fromNumber: string): void {
  const sockets = agentSockets.get(agentId);
  if (!sockets) return;
  const payload = JSON.stringify({ type: 'inbound_call', callId, fromNumber });
  for (const ws of sockets) {
    try { ws.send(payload); } catch { /* ignore closed socket */ }
  }
}
