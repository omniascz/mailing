...
const phoneNumberRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/api/v1/phone/numbers/search', {
  ...
  app.post('/api/v1/phone/numbers', {
    ...
    const { providerSid, monthlyRateUsd } = await provisionNumberWithProvider(body.provider, body.number);
  ...
  app.patch('/api/v1/phone/numbers/:id', {
    preHandler: [app.authenticate, app.requireRole('admin')],
    schema: { tags: ['Phone Numbers'] },
  }, async (req, reply) => {
    ...
    const [row] = await db.update(phoneNumbers)
  ...
  app.delete('/api/v1/phone/numbers/:id', {
    preHandler: [app.authenticate, app.requireRole('admin')],
    schema: { tags: ['Phone Numbers'] },
  }, async (req, reply) => {
    const { id } = req.params as { id: string };
    const [num] = await db.select().from(phoneNumbers)
      .where(and(eq(phoneNumbers.id, id), eq(phoneNumbers.orgId, req.user!.orgId))).limit(1);
  ...
  app.get('/api/v1/phone/port-requests', {
    preHandler: [app.authenticate],
    schema: { tags: ['Phone Numbers'] },
  }, async (req, reply) => {
    const rows = await db.select().from(phoneNumberPortRequests)
      .where(eq(phoneNumberPortRequests.orgId, req.user!.orgId));
    return reply.send({ data: rows });
  });

  app.post('/api/v1/phone/port-requests', {
    preHandler: [app.authenticate, app.requireRole('admin')],
    schema: { tags: ['Phone Numbers'] },
  }, async (req, reply) => {
    const body = z.object({
  ...
  app.patch('/api/v1/phone/port-requests/:id', {
    preHandler: [app.authenticate, app.requireRole('admin')],
    schema: { tags: ['Phone Numbers'] },
  }, async (req, reply) => {
    const { id } = req.params as { id: string };
    ...
    const [row] = await db.update(phoneNumberPortRequests)
      .set({ ...body, targetDate: body.targetDate ? new Date(body.targetDate) : undefined, updatedAt: new Date() })
      .where(and(eq(phoneNumberPortRequests.id, id), eq(phoneNumberPortRequests.orgId, req.user!.orgId)))
      .returning();
...
async function searchAvailableNumbers(opts: {
  provider: string; countryCode: string; areaCode?: string;
  contains?: string; capabilities?: string[]; limit: number;
  ...
  if (opts.provider === 'twilio') {
    ...
    if (!res?.ok) return [];
    const data = (await res.json()) as { available_phone_numbers?: Array<{ phone_number: string; monthly_rental_rate?: string }> };
...
async function provisionNumberWithProvider(provider: string, number: string): Promise<{ providerSid: string | null; monthlyRateUsd: string | null }> {
  if (provider === 'twilio') {
    const sid = process.env.TWILIO_ACCOUNT_SID;
    const token = process.env.TWILIO_AUTH_TOKEN;
...
async function releaseNumberWithProvider(provider: string, providerSid: string | null): Promise<void> {
