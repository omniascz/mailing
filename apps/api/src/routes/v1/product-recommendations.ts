...
import { z } from 'zod';
import {
  alsoBought, topInCategory, personalisedFor,
  upsertProduct, listProducts, bulkUpsertProducts,
...
const productRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/api/v1/products/recommendations', {
    ...
    if (q.contactId) return reply.send({ data: await personalisedFor(req.user!.orgId, q.contactId, q.limit) });
    if (q.sku)       return reply.send({ data: await alsoBought(req.user!.orgId, q.sku, q.limit) });
    if (q.category)  return reply.send({ data: await topInCategory(req.user!.orgId, q.category, q.limit) });
    return reply.code(400).send({ code: 'MISSING_FILTER', message: 'Provide contactId, sku, or category' });
  });
};
