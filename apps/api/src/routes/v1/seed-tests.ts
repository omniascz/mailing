...
export default async function seedTestRoutes(app: FastifyInstance) {
  ...
  app.post('/api/v1/seed-addresses', { schema: { tags: ['Deliverability'] } }, async (req, reply) => {
    const body = z
  ...
  app.delete('/api/v1/seed-addresses/:id', { schema: { tags: ['Deliverability'] } }, async (req, reply) => {
  ...
  // ── Seed tests ─────────────────────────────────────────────────────────────
  app.post('/api/v1/seed-tests', { schema: { tags: ['Deliverability'], summary: 'Send content to seed inboxes' } }, async (req, reply) => {
    const body = z
      .object({
        from: z.string().email(),
  ...
  app.get('/api/v1/seed-tests/:id', { schema: { tags: ['Deliverability'], summary: 'Seed test placement results' } }, async (req) => {
    const { id } = idParam.parse(req.params);
  ...
  app.post('/api/v1/seed-tests/:id/results', { schema: { tags: ['Deliverability'], summary: 'Record real seed placement' } }, async (req) => {
    const { id } = idParam.parse(req.params);
    const body = z
