...
import { z } from 'zod';
import { inferPersona, upsertPersona, getPersona, listPersonaSegment } from '../../services/ai/persona-inference.js';

export default async function personasRoutes(app: FastifyInstance) {
  app.addHook('preHandler', app.requireAuth);

  app.get('/api/v1/personas/segments', async (req) => {
    const q = z.object({ persona: z.string(), limit: z.coerce.number().int().min(1).max(500).default(100) }).parse(req.query);
  ...
  app.post('/api/v1/personas/:contactId/infer', async (req) => {
    const { contactId } = z.object({ contactId: z.string().uuid() }).parse(req.params);
    const body = z.object({ useAi: z.boolean().default(true) }).optional().parse(req.body);
