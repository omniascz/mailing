...
import { z } from 'zod';
import { listProducts, getProduct, createProduct, updateProduct, deleteProduct } from '../../../services/commerce/products.js';
...
const commerceProductRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/api/v1/commerce/products', auth, async (req, reply) => {
    const q = z.object({
      active: z.enum(['true', 'false']).optional().transform(v => v != null ? v === 'true' : undefined),
