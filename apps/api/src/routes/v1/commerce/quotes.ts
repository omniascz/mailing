...
import {
  ...
  sendQuote, getQuoteByToken, convertQuoteToInvoice,
} from '../../../services/commerce/quotes.js';
import { initiateESign, completeBuiltinSigning, handleESignWebhook } from '../../../services/commerce/e-signature.js';
...
const commerceQuoteRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/api/v1/commerce/quotes', auth, async (req, reply) => {
    ...
    const data = await createQuote(req.user!.orgId, { ...body, lineItems: items, validUntil: body.validUntil ? new Date(body.validUntil) : undefined });
  ...
  app.post('/api/v1/commerce/quotes/:quoteId/esign', auth, async (req, reply) => {
    ...
    const data = await initiateESign(req.user!.orgId, quoteId, body.recipientEmail, body.recipientName);
    return reply.send({ data });
  });
  ...
  app.post('/api/v1/commerce/quotes/:quoteId/convert-to-invoice', auth, async (req, reply) => {
    ...
    const data = await convertQuoteToInvoice(req.user!.orgId, quoteId, body.dueDate ? new Date(body.dueDate) : undefined);
    return reply.code(201).send({ data });
  });
  ...
  app.post('/api/v1/webhooks/esign/:provider', async (req, reply) => {
    const { provider } = z.object({ provider: z.enum(['docusign', 'hellosign']) }).parse(req.params);
