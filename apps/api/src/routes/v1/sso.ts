...
import { z } from 'zod';
import {
  configureSso, getSsoConfig, deleteSsoConfig,
  buildLoginUrl, processOidcCallback, processSamlAcs,
  getSpMetadataXml,
...
const ssoRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/api/v1/sso/:orgId/login', {
    schema: { tags: ['SSO'] },
  }, async (req, reply) => {
  ...
  app.patch('/api/v1/sso/saml/attribute-mapping', {
    preHandler: [app.authenticate, app.requireRole('owner', 'admin')],
    ...
    if (!existing || existing.type !== 'saml') {
      return reply.status(400).send({ code: 'BAD_REQUEST', message: 'No SAML configuration found' });
    }
